...
// Reconfigurable slices of the editor configuration. Keeping language, edit
// state, and line numbers in compartments lets a prop change reconfigure the
// live EditorView instead of destroying and recreating it, which would reset
// the current draft text and the user's selection to the initial value.
function getLanguageExtension(language: BundledLanguage | string): Extension {
...
function getLineNumbersExtension(showLineNumbers: boolean): Extension {
  return showLineNumbers ? lineNumbers() : []
}

function getStaticExtensions(
  handleKeyDown: (event: globalThis.KeyboardEvent) => void
): Extension[] {
  return [
...
export function CodeMirrorCodeView({
  ariaLabel,
  autoFocus = false,
  language,
  onChange,
  ...
  // Compartments are created once per mounted editor so later prop changes can
  // reconfigure them in place. They live in refs to stay stable across renders.
  ...
  // Create the editor exactly once for the lifetime of the mount. Config props
  // are applied through compartment reconfiguration in the effects below, so
  // this effect intentionally does not depend on them: recreating the view here
  // would discard the live document and selection.
  useEffect(() => {
    ...
    const editorView = new EditorView({
      ...
      extensions: [
        ...
        ...getStaticExtensions((event) => onKeyDownRef.current?.(event)),
  ...
  // Reconfigure read-only/editable state without recreating the editor.
  useEffect(() => {
    const editorView = editorViewRef.current
    const editableCompartment = editableCompartmentRef.current
    ...
    editorView.dispatch({
      effects: editableCompartment.reconfigure(getEditableExtension(readOnly)),
    })
  }, [readOnly])

  // Reconfigure the line-number gutter without recreating the editor.
  useEffect(() => {
    const editorView = editorViewRef.current
    const lineNumbersCompartment = lineNumbersCompartmentRef.current
    if (!editorView || !lineNumbersCompartment) {
      return
    }
    editorView.dispatch({
      effects: lineNumbersCompartment.reconfigure(
        getLineNumbersExtension(showLineNumbers)
      ),
    })
  }, [showLineNumbers])
