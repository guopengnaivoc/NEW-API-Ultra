...
var getSystemPerformanceMonitorConfig = common.GetPerformanceMonitorConfig
var getCurrentSystemStatus = common.GetSystemStatus
...
func checkSystemPerformance() *types.NewAPIError {
	config := getSystemPerformanceMonitorConfig()
	if !config.Enabled {
		return nil
	}
	...
	if config.CPUThreshold > 0 && int(status.CPUUsage) > config.CPUThreshold {
		return types.NewErrorWithStatusCode(
			errors.New("system resources are currently overloaded"),
