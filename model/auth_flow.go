...
const (
	...
	AuthFlowPurposeEmailVerification = "email_verification"
	AuthFlowPurposePasswordReset     = "password_reset"
...
type AuthFlow struct {
	...
	ExpiresAt  time.Time  `json:"expires_at" gorm:"not null;index:idx_auth_flow_purpose_expiry;index:idx_auth_flow_expires_at"`
...
// CreateAuthFlowWithTx creates a flow through the caller's transaction handle.
// A nil handle is rejected so transaction-owned operations never fall back to DB.
...
func ClaimExternalAuthAssertionWithTx(tx *gorm.DB, purpose, assertion string, expiresAt time.Time) error {
	return ClaimExternalAuthAssertionWithTxAt(tx, purpose, assertion, expiresAt, time.Now())
}

// ClaimExternalAuthAssertionWithTxAt records a provider assertion using the
// caller's transaction operation time. This keeps assertion expiry checks
// aligned with the other state changes in a larger authentication ceremony.
func ClaimExternalAuthAssertionWithTxAt(tx *gorm.DB, purpose, assertion string, expiresAt, now time.Time) error {
	purpose = strings.TrimSpace(purpose)
	assertion = strings.TrimSpace(assertion)
	if tx == nil || purpose == "" || assertion == "" || now.IsZero() || !expiresAt.After(now) {
...
func ConsumeAuthFlowWithAction(token string, match AuthFlowMatch, action func(tx *gorm.DB, flow *AuthFlow) error) (*AuthFlow, error) {
	return ConsumeAuthFlowWithActionAt(token, match, time.Now(), action)
}

// ConsumeAuthFlowWithActionAt consumes a flow and runs action at one explicit
// transaction operation time. An action failure rolls the consumption back.
func ConsumeAuthFlowWithActionAt(token string, match AuthFlowMatch, now time.Time, action func(tx *gorm.DB, flow *AuthFlow) error) (*AuthFlow, error) {
	if token == "" || match.Purpose == "" || now.IsZero() {
		return nil, ErrAuthFlowInvalid
	}
...
func DeleteAuthFlow(token, purpose string) error {
	token = strings.TrimSpace(token)
	purpose = strings.TrimSpace(purpose)
	if token == "" || purpose == "" {
		return ErrAuthFlowInvalid
	}
	return DB.Where("token_hash = ? AND purpose = ?", authFlowTokenHash(token), purpose).
		Delete(&AuthFlow{}).Error
}
