...
export function Home() {
  ...
  // Target only the configured iframe origin so preference payloads are not
  // delivered to whatever document the frame may have navigated to.
  const iframeOrigin = useMemo(() => {
    if (!isUrl) return null
    try {
      return new URL(content).origin
    } catch {
      return null
    }
  }, [content, isUrl])
  ...
  }, [i18n.language, resolvedTheme, iframeOrigin])
