...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string) *types.NewAPIError {
	...
	if info.RelayFormat == types.RelayFormatClaude {
		...
		response, conversionErr := relayconvert.StreamResponseClaude2OpenAIValidated(&claudeResponse)
		if conversionErr != nil {
			if converror.IsUpstreamResponse(conversionErr) {
			...
			return types.NewError(conversionErr, types.ErrorCodeBadResponseBody)
		}
