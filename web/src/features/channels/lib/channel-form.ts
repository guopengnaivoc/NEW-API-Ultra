...
import {
  ...
  validateVertexServiceAccountValue,
...
export function getVertexCredentialPresentation(options: {
  channelType: number
  targetVertexKeyType: 'json' | 'api_key'
  hasExistingMultiKey: boolean
  hasExistingVertexServiceAccountKey: boolean
  currentKeyMode?: 'append' | 'replace'
  forcedRestoreKeyMode?: 'append' | null
}): {
  multiKeyMode: 'single' | 'multi_to_single'
  requiresReplacement: boolean
  keyMode: 'append' | 'replace' | undefined
  forcedRestoreKeyMode: 'append' | null | undefined
} {
  const targetsServiceAccounts =
    options.channelType === 41 && options.targetVertexKeyType === 'json'
  const sourceUsesServiceAccounts =
    options.hasExistingVertexServiceAccountKey === true
  const changesCredentialFormat =
    options.hasExistingMultiKey &&
    targetsServiceAccounts !== sourceUsesServiceAccounts
  let keyMode = options.currentKeyMode
  let forcedRestoreKeyMode = options.forcedRestoreKeyMode

  if (keyMode && changesCredentialFormat) {
    if (keyMode !== 'replace') {
      forcedRestoreKeyMode ??= keyMode
      keyMode = 'replace'
    }
  } else if (keyMode && forcedRestoreKeyMode) {
    keyMode = forcedRestoreKeyMode
    forcedRestoreKeyMode = null
  }

  return {
    multiKeyMode:
      targetsServiceAccounts && options.hasExistingMultiKey
        ? 'multi_to_single'
        : 'single',
    requiresReplacement: changesCredentialFormat,
    keyMode,
    forcedRestoreKeyMode,
  }
}

export function getVertexCredentialSynchronization(options: {
  isEditing: boolean
  ...
  forcedRestoreKeyMode: 'append' | null
}): ReturnType<typeof getVertexCredentialPresentation> & {
  shouldSynchronize: boolean
} {
  const presentation = getVertexCredentialPresentation({
    channelType: options.channelType,
    targetVertexKeyType: options.targetVertexKeyType,
    hasExistingMultiKey: options.hasExistingMultiKey,
    hasExistingVertexServiceAccountKey:
      options.hasExistingVertexServiceAccountKey,
    currentKeyMode: options.keyMode,
    forcedRestoreKeyMode: options.forcedRestoreKeyMode,
  })
  const hasTargetPresentationMismatch =
    options.multiKeyMode !== undefined &&
    options.multiKeyMode !== presentation.multiKeyMode

  return {
    ...presentation,
    shouldSynchronize:
      options.isEditing &&
      (options.channelType === 41 ||
        options.sourceChannelType === 41 ||
        options.hasExistingVertexServiceAccountKey ||
        options.forcedRestoreKeyMode !== null ||
        hasTargetPresentationMismatch),
  }
}

const SINGLE_REPLACEMENT_KEY_ERROR =
  'A single replacement key is required when leaving Vertex AI service-account mode'

function getSingleReplacementKeyError(
  formData: Pick<
    ChannelFormValues,
    | 'type'
    | 'vertex_key_type'
    | 'has_existing_multi_key'
    | 'has_existing_vertex_service_account_key'
    | 'key'
  >
): string | undefined {
  const presentation = getVertexCredentialPresentation({
    channelType: formData.type,
    targetVertexKeyType: formData.vertex_key_type || 'json',
    hasExistingMultiKey: formData.has_existing_multi_key === true,
    hasExistingVertexServiceAccountKey:
      formData.has_existing_vertex_service_account_key === true,
  })
  const nonEmptyLines = String(formData.key || '')
    .split(/\r?\n/)
    .filter((line) => line.trim().length > 0)

  if (
    presentation.requiresReplacement &&
    presentation.multiKeyMode === 'single' &&
    nonEmptyLines.length > 1
  ) {
    return SINGLE_REPLACEMENT_KEY_ERROR
  }
  return undefined
}
...
export const channelFormSchema = z
  .object({
    ...
    has_existing_vertex_service_account_key: z.boolean().optional(),
    has_existing_multi_key: z.boolean().optional(),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.type === 41 && data.vertex_key_type === 'json') {
      if (!data.key?.trim()) {
        if (!data.has_existing_vertex_service_account_key) {
          addRequiredIssue(
            ctx,
            'key',
            'Vertex AI service account key is required when enabling JSON service-account mode'
          )
        }
      } else {
        const batchMode =
        ...
        const existingMultiKeyEdit =
          data.multi_key_mode === 'multi_to_single' &&
          data.has_existing_vertex_service_account_key === true
        try {
          validateVertexServiceAccountValue(data.key, {
            batch: batchMode,
            allowSingle: existingMultiKeyEdit,
          })
        } catch (error) {
          let message = 'Vertex AI service account key must be valid JSON'
          if (error instanceof VertexServiceAccountError) {
            switch (error.code) {
              case 'invalid_schema':
                message =
                  'Vertex AI service account key must include type, project_id, private_key, and client_email'
                break
              case 'invalid_mode':
                message = batchMode
                  ? 'Vertex AI service account key must be a JSON array in batch modes'
                  : 'Vertex AI service account key must be a single JSON object in single mode'
                break
              case 'too_many_keys':
                message =
                  'Vertex AI service account key count exceeds the 100-key limit'
                break
              case 'key_too_large':
                message =
                  'A Vertex AI service account key exceeds the 64 KiB limit'
                break
          ...
          addRequiredIssue(ctx, 'key', message)
        }
      }
    }

    const leavesExistingVertexServiceAccount =
      data.has_existing_vertex_service_account_key === true &&
      !(data.type === 41 && data.vertex_key_type === 'json')
    if (leavesExistingVertexServiceAccount) {
      if (!data.key?.trim()) {
        addRequiredIssue(ctx, 'key', ERROR_MESSAGES.REQUIRED_KEY)
      }
    }

    if (data.type === 41 && data.vertex_key_type === 'api_key') {
      const replacesExistingServiceAccounts =
        data.has_existing_vertex_service_account_key === true &&
        Boolean(data.key?.trim()) &&
        data.key_mode === 'replace'
      if (data.has_existing_vertex_service_account_key && !data.key?.trim()) {
        addRequiredIssue(ctx, 'key', ERROR_MESSAGES.REQUIRED_KEY)
      }
      if (
        data.multi_key_mode &&
        data.multi_key_mode !== 'single' &&
        !replacesExistingServiceAccounts
      ) {
        addRequiredIssue(
          ctx,
          'multi_key_mode',
    ...
    const vertexPresentation = getVertexCredentialPresentation({
      channelType: data.type,
      targetVertexKeyType: data.vertex_key_type || 'json',
      hasExistingMultiKey: data.has_existing_multi_key === true,
      hasExistingVertexServiceAccountKey:
        data.has_existing_vertex_service_account_key === true,
    })
    if (vertexPresentation.requiresReplacement && data.key_mode !== 'replace') {
      addRequiredIssue(
        ctx,
        'key_mode',
    ...
    const singleReplacementKeyError = getSingleReplacementKeyError(data)
    if (singleReplacementKeyError) {
      addRequiredIssue(ctx, 'key', singleReplacementKeyError)
    }
...
export const CHANNEL_FORM_DEFAULT_VALUES: ChannelFormValues = {
  ...
  key_mode: 'append',
  has_existing_vertex_service_account_key: false,
  has_existing_multi_key: false,
...
export function transformChannelToFormDefaults(
  ...
  const storedSetting = parseStoredJsonObject(channel.setting)
  const storedSettings = parseStoredJsonObject(channel.settings)
  ...
  const storedProtocol =
    typeof storedSetting.http_protocol === 'string'
      ? storedSetting.http_protocol
      : undefined
  const protocol = normalizeHttpProtocol(storedProtocol)
  const storedShards = storedSetting.http2_connection_shards
  const shards =
    typeof storedShards === 'number' && Number.isInteger(storedShards)
      ? normalizeHttp2ConnectionShards(storedShards)
      : 1

  extraSettings = {
    force_format: storedSetting.force_format === true,
    thinking_to_content: storedSetting.thinking_to_content === true,
    proxy: typeof storedSetting.proxy === 'string' ? storedSetting.proxy : '',
    http_protocol: protocol,
    http2_connection_shards: protocol === HTTP_PROTOCOL_HTTP1 ? 1 : shards,
    pass_through_body_enabled: storedSetting.pass_through_body_enabled === true,
    system_prompt:
      typeof storedSetting.system_prompt === 'string'
        ? storedSetting.system_prompt
        : '',
    system_prompt_override: storedSetting.system_prompt_override === true,
  }
  ...
  if (channel.type === 41 && storedSettings.vertex_key_type === 'api_key') {
    vertexKeyType = 'api_key'
  }
  azureResponsesVersion =
    typeof storedSettings.azure_responses_version === 'string'
      ? storedSettings.azure_responses_version
      : ''
  isEnterpriseAccount = storedSettings.openrouter_enterprise === true
  awsKeyType = storedSettings.aws_key_type === 'api_key' ? 'api_key' : 'ak_sk'
  allowServiceTier = storedSettings.allow_service_tier === true
  disableStore = storedSettings.disable_store === true
  allowSafetyIdentifier = storedSettings.allow_safety_identifier === true
  allowIncludeObfuscation = storedSettings.allow_include_obfuscation === true
  allowInferenceGeo = storedSettings.allow_inference_geo === true
  allowSpeed = storedSettings.allow_speed === true
  claudeBetaQuery = storedSettings.claude_beta_query === true
  disableTaskPollingSleep = storedSettings.disable_task_polling_sleep === true
  upstreamModelUpdateCheckEnabled =
    storedSettings.upstream_model_update_check_enabled === true
  upstreamModelUpdateAutoSyncEnabled =
    storedSettings.upstream_model_update_auto_sync_enabled === true
  upstreamModelUpdateIgnoredModels = Array.isArray(
    storedSettings.upstream_model_update_ignored_models
  )
    ? storedSettings.upstream_model_update_ignored_models.join(',')
    : ''
  if (storedSettings.advanced_custom) {
    advancedCustom = stringifyAdvancedCustomConfig(
      storedSettings.advanced_custom
    )
  }

  const vertexPresentation = getVertexCredentialPresentation({
    channelType: channel.type,
    targetVertexKeyType: vertexKeyType,
    hasExistingMultiKey: channel.channel_info.is_multi_key,
    hasExistingVertexServiceAccountKey:
      channel.type === 41 && vertexKeyType === 'json',
  })
  ...
  return {
    ...
    setting: JSON.stringify(storedSetting),
    param_override: channel.param_override || '',
    header_override: channel.header_override || '',
    settings: JSON.stringify(storedSettings),
    other: channel.other || '',
    multi_key_mode: vertexPresentation.multiKeyMode,
    ...
    has_existing_vertex_service_account_key:
      channel.type === 41 && vertexKeyType === 'json',
    has_existing_multi_key: channel.channel_info.is_multi_key,
    // Channel extra settings
    ...extraSettings,
    // Type-specific settings
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  const settingsObj = parseStoredJsonObject(formData.settings)
...
export function transformFormDataToUpdatePayload(
  ...
  const singleReplacementKeyError = getSingleReplacementKeyError(formData)
  if (singleReplacementKeyError) {
    throw new Error(singleReplacementKeyError)
  }
