...
func settleTestQuota(info *relaycommon.RelayInfo, priceData hosttypes.PriceData, usage *dto.Usage) (int, *billingexpr.TieredResult) {
	...
	if !priceData.UsePrice {
		completionQuota, completionClamp := common.QuotaRoundChecked(
			float64(usage.CompletionTokens) * priceData.CompletionRatio,
		)
		if info != nil && info.QuotaClamp == nil && completionClamp != nil {
			info.QuotaClamp = completionClamp
		}

		quota, ratioClamp := common.QuotaRoundChecked(
			(float64(usage.PromptTokens) + float64(completionQuota)) *
				priceData.ModelRatio,
		)
		if info != nil && info.QuotaClamp == nil && ratioClamp != nil {
			info.QuotaClamp = ratioClamp
		}
		if ratioClamp == nil && priceData.ModelRatio != 0 && quota <= 0 {
			quota = 1
		}
		return quota, nil
	}
