...
func InitEnv() {
	...
	// TLS_INSECURE_SKIP_VERIFY is honored only by the dedicated relay/
	// fetch/sync transports that check common.TLSInsecureSkipVerify.
	// It must not mutate http.DefaultTransport: that silently disabled
	// certificate verification for every default-client consumer
	// (payment callbacks, OAuth token exchanges, telemetry, SDKs).
	TLSInsecureSkipVerify = GetEnvOrDefaultBool("TLS_INSECURE_SKIP_VERIFY", false)
	if TLSInsecureSkipVerify {
		SysLog("TLS_INSECURE_SKIP_VERIFY=true: upstream relay/fetch clients skip TLS verification; payment/OAuth default clients keep verifying")
	}
...
func initUserSessionSettings() {
	...
	UserSessionIdleTimeoutSeconds = int64(positiveUserSessionEnv("USER_SESSION_IDLE_TIMEOUT_SECONDS", DefaultUserSessionIdleTimeoutSeconds))
...
func initConstantEnv() {
	...
	// Default 16 MiB per SSE line; values above 64 MiB are clamped by the
	// scanner (relay/helper.MaxScannerBufferMB).
