...
func GetClaudeSettings() *ClaudeSettings {
	current := config.Snapshot[ClaudeSettings]("claude")
	if _, ok := current.DefaultMaxTokens["default"]; ok {
		return current
	}

	derived := *current
	derived.DefaultMaxTokens = make(map[string]int, len(current.DefaultMaxTokens)+1)
	for model, maxTokens := range current.DefaultMaxTokens {
		derived.DefaultMaxTokens[model] = maxTokens
	}
	derived.DefaultMaxTokens["default"] = 8192
	return &derived
}
