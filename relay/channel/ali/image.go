...
func updateTask(info *relaycommon.RelayInfo, taskID string) (*AliResponse, error, []byte) {
	return updateTaskWithContext(context.Background(), info, taskID)
}

func updateTaskWithContext(requestCtx context.Context, info *relaycommon.RelayInfo, taskID string) (*AliResponse, error, []byte) {
	if requestCtx == nil {
	...
	if resp == nil {
		return &aliResponse, errors.New("nil response"), nil
	}
...
func asyncTaskWait(c *gin.Context, info *relaycommon.RelayInfo, taskID string, requestCtx context.Context) (*AliResponse, []byte, error) {
	if requestCtx == nil {
		requestCtx = context.Background()
	}
	waitSeconds := 10
	...
	if err := sleepWithContext(requestCtx, 5*time.Second); err != nil {
		return nil, nil, err
	}
	...
	for {
		...
		step++
		rsp, err, body := updateTaskWithContext(requestCtx, info, taskID)
		responseBody = body
		if err != nil {
			logger.LogWarn(c, "asyncTaskWait UpdateTask err: "+err.Error())
			if err := sleepWithContext(requestCtx, time.Duration(waitSeconds)*time.Second); err != nil {
				return nil, nil, err
			}
			continue
		}
...
func aliImageHandler(a *Adaptor, c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*types.NewAPIError, *dto.Usage) {
	...
	if a.IsSyncImageModel {
		...
		// 异步图片模型需要轮询任务结果
		aliResponse, originRespBody, err = asyncTaskWait(c, info, aliTaskResponse.Output.TaskId, pollCtx)
		if err != nil {
