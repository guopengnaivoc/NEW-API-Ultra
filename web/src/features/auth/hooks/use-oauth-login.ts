...
export function useOAuthLogin(
  ...
  const { handleLoginSuccess, beginTwoFAChallenge } = useAuthRedirect()
  ...
  const handleOIDCLogin = async () => {
    ...
    try {
      ...
      if (!isSafeExternalHttpUrl(url)) {
        throw new Error('unsafe authorization endpoint')
      }
      window.open(url, '_self')
    } catch {
  ...
  const handleTelegramAuthorization = async (value: unknown) => {
    ...
    try {
      ...
      if (beginTwoFAChallenge(response.data)) {
  ...
  const handleCustomOAuthLogin = async (provider: CustomOAuthProviderInfo) => {
    ...
    try {
      ...
      if (!isSafeExternalHttpUrl(url.toString())) {
        throw new Error('unsafe authorization endpoint')
      }
