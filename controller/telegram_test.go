...
func TestCommitTelegramBindAtIdleDeadlineRollsBackAndReconciles(t *testing.T) {
	previousDB := model.DB
	...
	common.SessionSecret = "telegram-idle-commit-test-secret"
	...
	operationTime := time.Unix(2_001_000_000, 500_000_000)
	lastActiveAt := operationTime.Unix() - common.UserSessionIdleTimeoutSeconds
	user := &model.User{
		Username:    "telegram-idle-commit-user",
		Password:    "password-placeholder",
		Role:        common.RoleCommonUser,
		Status:      common.UserStatusEnabled,
		Group:       "default",
		AuthVersion: 1,
		AffCode:     "telegram-idle-commit-user",
	}
	require.NoError(t, db.Create(user).Error)
	session := &model.UserSession{
		SID:                 "telegram-idle-commit-session",
		UserID:              user.Id,
		Version:             1,
		UserAuthVersion:     user.AuthVersion,
		Status:              model.UserSessionStatusActive,
		RefreshHash:         "telegram-idle-current-hash",
		PreviousRefreshHash: "telegram-idle-previous-hash",
		PreviousValidUntil:  operationTime.Unix() + 30,
		LoginMethod:         "password",
		CreatedAt:           lastActiveAt,
		LastActiveAt:        lastActiveAt,
		ExpiresAt:           operationTime.Unix() + 3600,
	}
	require.NoError(t, model.CreateUserSession(session))
	flowToken, _, err := model.CreateAuthFlow(model.AuthFlowCreate{
		...
		ExpiresAt: operationTime.Add(time.Hour),
	})
	require.NoError(t, err)

	prechecked, err := model.GetActiveUserSessionBySIDAt(session.SID, operationTime.Unix()-1)
	require.NoError(t, err)
	require.NotNil(t, prechecked)
	assert.Equal(t, model.UserSessionStatusActive, prechecked.Status)

	err = commitTelegramBindAt(telegramBindCommit{
		FlowToken:          flowToken,
		UserID:             user.Id,
		SessionID:          session.SID,
		TelegramID:         "telegram-idle-subject",
		Assertion:          "telegram-idle-assertion",
		AssertionExpiresAt: operationTime.Add(time.Minute),
	}, operationTime)

	assert.ErrorIs(t, err, service.ErrLoginSessionRevoked)
	var storedUser model.User
	require.NoError(t, db.First(&storedUser, user.Id).Error)
	assert.Empty(t, storedUser.TelegramId)
	assert.Equal(t, int64(1), storedUser.AuthVersion)
	var identityClaimCount int64
	require.NoError(t, db.Model(&model.ExternalIdentityClaim{}).
		Where("provider = ?", model.ExternalIdentityProviderTelegram).
		Count(&identityClaimCount).Error)
	assert.Zero(t, identityClaimCount)
	var assertionCount int64
	require.NoError(t, db.Model(&model.AuthFlow{}).
		Where("purpose = ?", model.AuthFlowPurposeTelegramAssertion).
		Count(&assertionCount).Error)
	assert.Zero(t, assertionCount)
	storedFlow, err := model.GetAuthFlow(flowToken, model.AuthFlowMatch{
		Purpose:   model.AuthFlowPurposeTelegramBind,
		UserId:    user.Id,
		SessionId: session.SID,
	})
	...
	require.NoError(t, db.First(&storedSession, "sid = ?", session.SID).Error)
	assert.Equal(t, model.UserSessionStatusRevoked, storedSession.Status)
	assert.Equal(t, operationTime.Unix(), storedSession.RevokedAt)
	assert.Equal(t, model.UserSessionRevokedReasonIdleTimeout, storedSession.RevokedReason)
	assert.Equal(t, "telegram-idle-current-hash", storedSession.RefreshHash)
	assert.Equal(t, "telegram-idle-previous-hash", storedSession.PreviousRefreshHash)
	assert.Equal(t, operationTime.Unix()+30, storedSession.PreviousValidUntil)
	assert.Equal(t, lastActiveAt, storedSession.LastActiveAt)
	assert.Equal(t, operationTime.Unix()+3600, storedSession.ExpiresAt)
}

func TestCommitTelegramBindAtIdleDeadlineDoesNotRevokeRefreshWinner(t *testing.T) {
	previousDB := model.DB
	previousType := common.MainDatabaseType()
	previousRedis := common.RedisEnabled
	previousSecret := common.SessionSecret
	previousIdleTimeout := common.UserSessionIdleTimeoutSeconds
	databasePath := filepath.Join(t.TempDir(), "telegram-refresh-winner.db")
	db, err := gorm.Open(
	...
	common.SetMainDatabaseType(common.DatabaseTypeSQLite)
	common.RedisEnabled = false
	common.SessionSecret = "telegram-refresh-winner-test-secret"
	common.UserSessionIdleTimeoutSeconds = int64(common.DefaultUserSessionIdleTimeoutSeconds)
	t.Cleanup(func() {
		model.DB = previousDB
		common.SetMainDatabaseType(previousType)
	...
	operationTime := time.Unix(2_001_200_000, 0)
	refreshTime := operationTime.Add(-time.Second)
	lastActiveAt := operationTime.Unix() - common.UserSessionIdleTimeoutSeconds
	user := &model.User{
		Username:    "telegram-refresh-winner-user",
		Password:    "password-placeholder",
		Role:        common.RoleCommonUser,
		Status:      common.UserStatusEnabled,
		Group:       "default",
		AuthVersion: 1,
		AffCode:     "telegram-refresh-winner-user",
	}
	require.NoError(t, db.Create(user).Error)
	session := &model.UserSession{
		SID:                 "telegram-refresh-winner-session",
		UserID:              user.Id,
		Version:             1,
		UserAuthVersion:     user.AuthVersion,
		Status:              model.UserSessionStatusActive,
		RefreshHash:         "telegram-refresh-winner-current",
		PreviousRefreshHash: "telegram-refresh-winner-previous",
		PreviousValidUntil:  operationTime.Unix() + 30,
		LoginMethod:         "password",
		CreatedAt:           lastActiveAt,
		LastActiveAt:        lastActiveAt,
	...
	reconciliationReadObserved := make(chan struct{})
	releaseReconciliationRead := make(chan struct{})
	var releaseOnce sync.Once
	release := func() {
		releaseOnce.Do(func() {
			close(releaseReconciliationRead)
		})
	}
	var sessionQueryCount atomic.Int64
	callbackName := "test:pause_telegram_idle_reconciliation_for_refresh_winner"
	require.NoError(t, db.Callback().Query().After("gorm:query").Register(callbackName, func(tx *gorm.DB) {
		if tx.Statement == nil || tx.Statement.Table != "user_sessions" ||
			sessionQueryCount.Add(1) != 2 {
			return
		}
		close(reconciliationReadObserved)
	...
	go func() {
		bindResult <- commitTelegramBindAt(telegramBindCommit{
			FlowToken:          flowToken,
			UserID:             user.Id,
			SessionID:          session.SID,
			TelegramID:         "telegram-refresh-winner-subject",
			Assertion:          "telegram-refresh-winner-assertion",
			AssertionExpiresAt: operationTime.Add(time.Minute),
		}, operationTime)
	}()

	select {
	case <-reconciliationReadObserved:
	case <-time.After(5 * time.Second):
		require.FailNow(t, "Telegram callback did not reach idle reconciliation")
	}
	rotated, err := model.RotateUserSessionRefresh(
		user.Id,
		session.SID,
		"telegram-refresh-winner-current",
		"telegram-refresh-winner-next",
		refreshTime.Unix(),
		30*time.Second,
	)
	require.NoError(t, err)
	require.NotNil(t, rotated)
	assert.Equal(t, refreshTime.Unix(), rotated.LastActiveAt)
	release()

	select {
	case err = <-bindResult:
	case <-time.After(5 * time.Second):
		require.FailNow(t, "Telegram callback did not finish")
	}
	assert.ErrorIs(t, err, service.ErrLoginSessionRevoked)
	...
	assert.Zero(t, storedSession.RevokedAt)
	assert.Empty(t, storedSession.RevokedReason)
	assert.Equal(t, "telegram-refresh-winner-next", storedSession.RefreshHash)
	assert.Equal(t, "telegram-refresh-winner-current", storedSession.PreviousRefreshHash)
	assert.Equal(t, refreshTime.Unix()+30, storedSession.PreviousValidUntil)
	assert.Equal(t, refreshTime.Unix(), storedSession.LastActiveAt)
	assert.Equal(t, operationTime.Unix()+3600, storedSession.ExpiresAt)
	assert.Equal(t, int64(1), storedSession.Version)
	assert.Equal(t, int64(1), storedSession.UserAuthVersion)
	...
	require.NoError(t, err)
	assert.Nil(t, storedFlow.ConsumedAt)
	assert.False(t, errors.Is(err, model.ErrAuthFlowConsumed))
}
...
func TestTelegramBindCommitsFlowAssertionAndBindingAtomically(t *testing.T) {
	...
	assert.Equal(t, int64(2), storedUser.AuthVersion)
	var advancedSession model.UserSession
	require.NoError(t, db.First(&advancedSession, "sid = ?", session.SID).Error)
	assert.Equal(t, int64(2), advancedSession.UserAuthVersion)
	assert.Equal(t, int64(2), advancedSession.Version)
	assert.Equal(t, model.UserSessionStatusActive, advancedSession.Status)
