...
func newLogRotationState(maxCount int) *logRotationState {
	return &logRotationState{maxCount: maxCount}
}

func (s *logRotationState) recordLog() bool {
	s.mu.Lock()
	defer s.mu.Unlock()

	s.count++
	if s.count <= s.maxCount || s.setupWorking {
		return false
	}
	s.count = 0
	s.setupWorking = true
...
func (s *logRotationState) tryStartSetup() bool {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *logRotationState) finishSetup() {
	s.mu.Lock()
	s.setupWorking = false
	s.mu.Unlock()
}

var logRotation = newLogRotationState(maxLogCount)
...
func logHelper(ctx context.Context, level string, msg string) {
	...
	if logRotation.recordLog() {
		gopool.Go(setupLoggerOwned)
	}
}
