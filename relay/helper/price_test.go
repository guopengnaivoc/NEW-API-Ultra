...
func TestModelPriceHelperFreezesAutoGroupBeforeRealtimeBilling(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.NoError(t, ratio_setting.UpdateModelRatioByJSONString(
		`{"realtime-auto-group-model":1}`,
	))
	require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(
		`{"default":1,"realtime-auto":2}`,
	))

	context, _ := gin.CreateTestContext(httptest.NewRecorder())
	context.Set("group", "default")
	context.Set("auto_group", "realtime-auto")
	info := &relaycommon.RelayInfo{
		OriginModelName: "realtime-auto-group-model",
		UserGroup:       "default",
		UsingGroup:      "default",
	}
	...
	require.NoError(t, err)
	require.Equal(t, "realtime-auto", info.UsingGroup)
	require.Equal(t, 2.0, priceData.GroupRatioInfo.GroupRatio)
	require.Equal(t, 2.0, info.PriceData.GroupRatioInfo.GroupRatio)

	require.NoError(t, ratio_setting.UpdateGroupRatioByJSONString(
		`{"default":1,"realtime-auto":0.5}`,
	))
	require.Equal(t, 2.0, priceData.GroupRatioInfo.GroupRatio)
	require.Equal(t, 2.0, info.PriceData.GroupRatioInfo.GroupRatio)
}
