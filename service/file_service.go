...
func getBase64ContextCacheKey(data string, mimeType string) string {
	// Hash the complete source. The inner digest has fixed length, so binding it
	// to the MIME type does not require copying a potentially large base64 value.
	contentDigest := common.GenerateHMAC(data)
	keyMaterial := mimeType + ":" + contentDigest
...
func LoadFileSource(c *gin.Context, source types.FileSource, reason ...string) (*types.CachedFileData, error) {
	if source == nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryInvalidSource, nil)
	}

	switch knownSource := source.(type) {
	case *types.URLSource:
		if knownSource == nil {
			return nil, types.NewFileSourceError(types.FileSourceErrorCategoryInvalidSource, knownSource)
		}
	...
	default:
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryUnsupported, source)
	}
	...
	case *types.URLSource:
		...
		cachedData, err = loadFromURL(c, s, reason...)
	case *types.Base64Source:
		if c != nil {
		...
		cachedData, err = loadFromBase64(s)
	default:
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryUnsupported, source)
	}
	...
	if err != nil {
		...
		if errors.As(err, &fileSourceError) {
			return nil, fileSourceError
		}
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryInternal, source)
	}
...
func loadFromURL(c *gin.Context, source *types.URLSource, reason ...string) (*types.CachedFileData, error) {
	// 下载文件
	...
	resp, err := DoDownloadRequest(source.URL, reason...)
	if err != nil {
		var fileSourceError *types.FileSourceError
		if errors.As(err, &fileSourceError) {
			return nil, fileSourceError
		}
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryDownloadFailed, source)
	}
	defer resp.Body.Close()

	if resp.StatusCode != 200 {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryUnexpectedStatus, source, resp.StatusCode)
	}
	...
	if err != nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryReadFailed, source)
	}
	if len(fileBytes) > maxFileSize {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryTooLarge, source, constant.MaxFileDownloadMB)
	}
	...
	// 智能获取 MIME 类型
	mimeType := smartDetectMimeType(resp, source.URL, fileBytes)
...
func loadFromBase64(source *types.Base64Source) (*types.CachedFileData, error) {
	var mimeType string
	var cleanBase64 string

	// 处理 data: 前缀
	if strings.HasPrefix(source.Base64Data, "data:") {
		idx := strings.Index(source.Base64Data, ",")
		if idx != -1 {
			header := source.Base64Data[:idx]
			cleanBase64 = source.Base64Data[idx+1:]
	...
	if err != nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryInvalidBase64, source)
	}
...
func GetImageConfig(c *gin.Context, source types.FileSource) (image.Config, string, error) {
	...
	if err != nil {
		return image.Config{}, "", types.NewFileSourceError(types.FileSourceErrorCategoryCacheReadFailed, source)
	}
	decodedData, err := base64.StdEncoding.DecodeString(base64Str)
	if err != nil {
		return image.Config{}, "", types.NewFileSourceError(types.FileSourceErrorCategoryInvalidBase64, source)
	}

	config, format, err := decodeImageConfig(decodedData)
	if err != nil {
		return image.Config{}, "", types.NewFileSourceError(types.FileSourceErrorCategoryInvalidContent, source)
	}
...
func GetBase64Data(c *gin.Context, source types.FileSource, reason ...string) (string, string, error) {
	...
	base64Str, err := cachedData.GetBase64Data()
	if err != nil {
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryCacheReadFailed, source)
	}
...
func GetMimeType(c *gin.Context, source types.FileSource) (string, error) {
	...
	case *types.URLSource:
		if knownSource == nil {
			return "", types.NewFileSourceError(types.FileSourceErrorCategoryInvalidSource, knownSource)
		}
	case *types.Base64Source:
		if knownSource == nil {
			return "", types.NewFileSourceError(types.FileSourceErrorCategoryInvalidSource, knownSource)
	...
	default:
		return "", types.NewFileSourceError(types.FileSourceErrorCategoryUnsupported, source)
	}
