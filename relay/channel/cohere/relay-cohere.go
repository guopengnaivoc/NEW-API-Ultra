...
func cohereStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// A single ordered channel closed by the producer is the completion
	// signal: a receive on a closed buffered channel drains every queued
	// event before reporting closed, so tail content can never be dropped by
	// a racy select between a data channel and a separate stop channel.
