...
/**
 * Clear all playground data. Sweeps by key prefix rather than the known key
 * list so that conversations, message versions, and reasoning stored under
 * legacy or future playground keys are also removed when the session ends.
 */
export function clearPlaygroundData(): void {
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key && key.startsWith(PLAYGROUND_STORAGE_KEY_PREFIX)) {
