...
const PendingTwoFASetupTTL = 24 * time.Hour

const twoFASecretDomain = "two_factors:secret"
...
func (t *TwoFA) BeforeSave(_ *gorm.DB) error {
	stored, err := common.SealDataEncryptionValue(twoFASecretDomain, t.Secret)
	if err != nil {
		return err
	}
	t.EncryptedSecret = stored
	return nil
}

func (t *TwoFA) AfterFind(_ *gorm.DB) error {
...
func (t *TwoFA) CreatePendingTwoFASetup() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		return t.createPendingTwoFASetupWithTx(tx)
	})
}

// CreatePendingTwoFASetupWithBackupCodes stores the unverified factor and its
// recovery credentials atomically so cleanup cannot observe partial setup.
func (t *TwoFA) CreatePendingTwoFASetupWithBackupCodes(codes []string) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := t.createPendingTwoFASetupWithTx(tx); err != nil {
			return err
		}
		return replaceBackupCodesWithTx(tx, t.UserId, codes)
	})
}

func (t *TwoFA) createPendingTwoFASetupWithTx(tx *gorm.DB) error {
	if t == nil || t.UserId <= 0 || t.IsEnabled {
		return errors.New("无效的2FA待验证设置")
	}
	var existing TwoFA
	if err := tx.Where("user_id = ?", t.UserId).First(&existing).Error; err == nil {
...
func (t *TwoFA) DeletePendingTwoFASetup() error {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		deleted, err := deletePendingTwoFASetupWithTx(
			tx,
			t.Id,
			t.UserId,
			time.Time{},
...
func (t *TwoFA) IsPendingSetupExpired(now time.Time) bool {
	return t != nil &&
		!t.IsEnabled &&
		!t.CreatedAt.After(now.Add(-PendingTwoFASetupTTL))
}

// ExpirePendingSetup removes this enrollment only when it is disabled and its
// fixed lifetime has elapsed. The factor delete claims the record before any
// backup codes are touched, so a concurrent enable winner remains intact.
func (t *TwoFA) ExpirePendingSetup(now time.Time) (bool, error) {
	if t == nil || t.Id == 0 || t.UserId <= 0 {
		return false, errors.New("2FA记录ID不能为空")
	}
	if !t.IsPendingSetupExpired(now) {
		return false, nil
	}
	var expired bool
...
type pendingTwoFACleanupCandidate struct {
	Id     int
	UserId int
}

// CleanupExpiredPendingTwoFASetups removes every expired pending factor
// visible in this pass using bounded keyset queries.
func CleanupExpiredPendingTwoFASetups(
	ctx context.Context,
	now time.Time,
	batchSize int,
) (int64, error) {
	...
	cutoff := now.Add(-PendingTwoFASetupTTL)
	var (
		lastID  int
		deleted int64
	)
	for {
		var candidates []pendingTwoFACleanupCandidate
		if err := db.Model(&TwoFA{}).
			Select("id", "user_id").
			Where("is_enabled = ? AND created_at <= ? AND id > ?", false, cutoff, lastID).
			Order("id").
			Limit(batchSize).
...
func deletePendingTwoFASetupWithTx(
	tx *gorm.DB,
	id int,
	userID int,
	cutoff time.Time,
	...
	query := tx.Unscoped().
		Where("id = ? AND user_id = ? AND is_enabled = ?", id, userID, false)
	if !cutoff.IsZero() {
		query = query.Where("created_at <= ?", cutoff)
	}
	result := query.Delete(&TwoFA{})
	if result.Error != nil {
	...
	// Backup codes are user-scoped rather than factor-scoped. This broad
	// delete is safe only because it shares the factor-claim transaction: a
	// replacement enrollment cannot observe the factor as absent until this
	// delete commits, while a stale candidate returns above without touching
	// codes.
	if err := tx.Unscoped().
		Where("user_id = ?", userID).
		Delete(&TwoFABackupCode{}).Error; err != nil {
		return false, err
	}
	return true, nil
}
...
func (t *TwoFA) EnableWithAuthVersion() error {
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&pending).
			Where("is_enabled = ? AND created_at > ?", false, cutoff).
...
func (t *TwoFA) ValidateTOTPAndUpdateUsage(code string) (bool, error) {
	...
	// 验证TOTP码
	now := time.Now()
	counter, ok := common.MatchTOTPCounter(t.Secret, code, now)
	if !ok {
		// 增加失败次数
	...
	// 拒绝同一时间步的重放：只有当持久化的计数器仍小于本次命中的
	// 计数器时才接受，条件更新同时天然串行化并发重放。
	claim := DB.Model(&TwoFA{}).
		Where("id = ? AND COALESCE(last_used_counter, 0) < ?", t.Id, counter).
		Updates(map[string]interface{}{
