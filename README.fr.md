...
> Référence amont conservée pour l’attribution : le dépôt source d’origine est [`QuantumNous/new-api`](https://github.com/QuantumNous/new-api) et son image historique est `calciumion/new-api:latest`. Ces instructions amont de clonage/image ne constituent pas le chemin de déploiement de cette publication.

> Il s’agit d’un instantané de publication stabilisé ; cela ne signifie pas que tous les problèmes connus sont résolus. Utilisez un tag exact et terminez les vérifications de déploiement avant toute exposition publique.
...
# Facultatif : vérifier ou modifier .env avant la validation
# nano .env

# Valider sans afficher les secrets résolus
...
Consultez [`DEPLOYMENT.md`](./DEPLOYMENT.md) pour une image préconstruite ou un conteneur unique. Le parcours Compose ci-dessus est recommandé : il configure PostgreSQL, Redis, des secrets de session/chiffrement stables et le trousseau de chiffrement des données. Ne copiez pas une commande `docker run` sans ces réglages et des volumes persistants.

</details>

---

🎉 Après le déploiement, visitez `http://127.0.0.1:3000` pour commencer à utiliser!
...
| `DATA_ENCRYPTION_KEYS` | Porte-clés racine `key-id=base64-key` séparés par des virgules pour les secrets réversibles ; chaque clé décodée doit faire exactement 32 octets et être identique sur tous les nœuds | - |
| `DATA_ENCRYPTION_ACTIVE_KEY_ID` | Identifiant de clé utilisé pour les nouvelles enveloppes et le re-chiffrement de la clé racine | - |
| `DATA_ENCRYPTION_ENABLE` | Contrôle du chiffrement au repos et des migrations ; ne le désactivez que pendant la préparation documentée des versions mixtes | `true` |
...
2. Suivez le [`DEPLOYMENT.md`](./DEPLOYMENT.md) de ce dépôt ou le
   [`tutoriel BaoTa`](./docs/installation/BT.md) : clonez `NEW-API-Ultra`,
   générez `.env`, puis démarrez les services Docker Compose de ce dépôt.
3. Le paquet **New-API** installé en un clic depuis la boutique BaoTa et les
   images amont sont uniquement des références ; ils ne déploient pas cet instantané.
...
**[Guide de déploiement](./DEPLOYMENT.md)** • **[Issues de cette publication](https://github.com/guopengnaivoc/NEW-API-Ultra/issues)** • **[Releases de cette publication](https://github.com/guopengnaivoc/NEW-API-Ultra/releases)** • **[Documentation amont](https://docs.newapi.pro/en/docs)** • **[Issues amont](https://github.com/Calcium-Ion/new-api/issues)** • **[Releases amont](https://github.com/Calcium-Ion/new-api/releases)**
