...
export function ApiKeysMutateDrawer({
  ...
  const { triggerRefresh, showOneTimeTokens } = useApiKeys()
  ...
  const onSubmit = async (data: ApiKeyFormValues) => {
    ...
    const createdTokens: OneTimeTokenEntry[] = []
    try {
      ...
      if (isUpdate && currentRow) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (result.success && result.data?.key && result.data.token) {
            createdTokens.push({
            ...
            toast.error(
              result.message ||
                t(
                  'The API key was not returned. Create a new key before closing this page.'
                )
            )
            break
          }
        ...
        if (createdTokens.length > 0) {
          ...
          onOpenChange(false)
          showOneTimeTokens(createdTokens)
          triggerRefresh()
        }
      }
    } catch {
