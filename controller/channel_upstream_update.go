...
func collectPendingUpstreamModelChanges(
	ctx context.Context,
...
func getFetchModelsResponseBody(method string, requestURL string, channel *model.Channel, headers http.Header) ([]byte, error) {
	return getFetchModelsResponseBodyWithContext(
		context.Background(),
		method,
		requestURL,
		channel,
...
func getFetchModelsResponseBodyWithContext(
	ctx context.Context,
	method string,
	requestURL string,
	channel *model.Channel,
	...
	request, err := newChannelManagementRequest(ctx, method, requestURL)
	if err != nil {
		return nil, err
	}
	...
	return readChannelManagementResponse(
		client,
		request,
		channelModelResponseMaxBytes,
	)
}

func fetchChannelUpstreamModelIDs(channel *model.Channel) ([]string, error) {
	return fetchChannelUpstreamModelIDsWithContext(
		context.Background(),
		channel,
	)
}

func fetchChannelUpstreamModelIDsWithContext(
	ctx context.Context,
	channel *model.Channel,
) ([]string, error) {
	...
	if channel.Type == constant.ChannelTypeOllama {
		...
		models, err := ollama.FetchOllamaModels(
			ctx,
			baseURL,
			key,
	...
	body, err := getFetchModelsResponseBodyWithContext(
		ctx,
		http.MethodGet,
		url,
		channel,
...
func fetchAdvancedCustomUpstreamModelIDs(
	ctx context.Context,
	channel *model.Channel,
	baseURL string,
) ([]string, error) {
...
func checkAndPersistChannelUpstreamModelUpdates(
	...
	return checkAndPersistChannelUpstreamModelUpdatesWithContext(
		context.Background(),
		channel,
		settings,
		force,
...
func checkAndPersistChannelUpstreamModelUpdatesWithContext(
	ctx context.Context,
	channel *model.Channel,
	settings *dto.ChannelOtherSettings,
	force bool,
	...
	pendingAddModels, pendingRemoveModels, fetchErr := collectPendingUpstreamModelChanges(
		ctx,
		channel,
		*settings,
	)
...
scanLoop:
	for {
		...
		for _, channel := range channels {
			...
			checkedChannels++
			modelsChanged, autoAdded, err := checkAndPersistChannelUpstreamModelUpdatesWithContext(
				ctx,
				channel,
				&settings,
				force,
