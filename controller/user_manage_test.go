...
func setupManageUserTestDB(t *testing.T) *gorm.DB {
	...
	require.NoError(t, db.AutoMigrate(
		...
		// Deleting a user purges its authentication surface, so every credential
		// table the purge touches must exist for the handler to succeed.
		&model.Token{}, &model.TwoFA{}, &model.TwoFABackupCode{}, &model.DashboardAccessToken{},
		&model.AuthFlow{}, &model.PasskeyCredential{}, &model.ExternalIdentityClaim{},
