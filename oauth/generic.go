...
const (
	...
	maxGenericOAuthResponseBytes int64 = 1 << 20
)
...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken request started", p.config.Slug)
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken request ready: auth_style=%d",
	...
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken failed: category=transport", p.config.Slug))
	...
	body, err := readGenericOAuthResponse(res.Body)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken failed: category=response_read", p.config.Slug))
		return nil, NewOAuthError(i18n.MsgOAuthTokenFailed, map[string]any{"Provider": p.config.Name})
	}

	bodyStr := string(body)
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken response_bytes=%d",
		p.config.Slug, len(body))
	...
	if err := common.Unmarshal(body, &tokenResponse); err != nil {
		...
		if parseErr != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken failed: category=response_parse", p.config.Slug))
			return nil, err
		}
	...
	if tokenResponse.Error != "" {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] ExchangeToken failed: category=provider_error",
			p.config.Slug))
	...
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken success: has_scope=%t",
		p.config.Slug, tokenResponse.Scope != "")
...
func (p *GenericOAuthProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	res, err := client.Do(req)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo failed: category=transport", p.config.Slug))
		return nil, NewOAuthErrorWithRaw(i18n.MsgOAuthConnectFailed, map[string]any{"Provider": p.config.Name}, err.Error())
	}
	defer res.Body.Close()
	...
	body, err := readGenericOAuthResponse(res.Body)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo failed: category=response_read", p.config.Slug))
		return nil, NewOAuthError(i18n.MsgOAuthGetUserErr, nil)
	}

	bodyStr := string(body)
	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo response_bytes=%d",
		p.config.Slug, len(body))
	...
	if userId == "" {
		logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo failed: category=empty_user_id", p.config.Slug))
		return nil, NewOAuthError(i18n.MsgOAuthUserInfoEmpty, map[string]any{"Provider": p.config.Name})
	}

	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo success: has_username=%t, has_display_name=%t, has_email=%t",
		p.config.Slug, username != "", displayName != "", email != "")

	policyRaw := strings.TrimSpace(p.config.AccessPolicy)
	if policyRaw != "" {
		policy, err := parseAccessPolicy(policyRaw)
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("[OAuth-Generic-%s] GetUserInfo failed: category=invalid_access_policy", p.config.Slug))
		...
		if !allowed {
			...
			logger.LogWarn(ctx, fmt.Sprintf("[OAuth-Generic-%s] access denied by policy", p.config.Slug))
...
func readGenericOAuthResponse(body io.Reader) ([]byte, error) {
	data, err := io.ReadAll(io.LimitReader(body, maxGenericOAuthResponseBytes+1))
	if err != nil {
		return nil, err
	}
	if int64(len(data)) > maxGenericOAuthResponseBytes {
		return nil, errors.New("OAuth provider response exceeds 1 MiB")
	}
	return data, nil
}
