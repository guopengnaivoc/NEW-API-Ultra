...
func zhipuStreamHandler(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	// One ordered event channel, closed by the producer, replaces the old
	// data/meta/stop channel trio: separate channels lose upstream ordering
	// between data and meta lines, and a separate stop channel races the
	// buffered queues on normal completion, letting Go's select drop queued
	// tail content or the final usage-bearing meta event. A receive on a
	// closed buffered channel drains everything first, so close is the
	// deterministic completion signal.
	type zhipuStreamEvent struct {
		payload string
		isMeta  bool
	}
	eventChan := make(chan zhipuStreamEvent, 16)
		...
		for scanner.Scan() {
			...
			for i, line := range lines {
				...
				if line[:5] == "data:" {
					...
					case <-ctx.Done():
						return
					case eventChan <- zhipuStreamEvent{payload: line[5:]}:
					}
					if i != len(lines)-1 {
						select {
						case <-ctx.Done():
							return
						case eventChan <- zhipuStreamEvent{payload: "\n"}:
						}
					}
				} else if line[:5] == "meta:" {
					select {
					case <-ctx.Done():
						return
					case eventChan <- zhipuStreamEvent{payload: line[5:], isMeta: true}:
					}
				}
			}
		}
	...
	c.Stream(func(w io.Writer) bool {
		...
		case event, ok := <-eventChan:
			...
			if event.isMeta {
				...
				err := json.Unmarshal([]byte(event.payload), &zhipuResponse)
				if err != nil {
			...
			response := streamResponseZhipu2OpenAI(event.payload)
			jsonResponse, err := json.Marshal(response)
			if err != nil {
				common.SysLog("error marshalling stream response: " + err.Error())
				return true
