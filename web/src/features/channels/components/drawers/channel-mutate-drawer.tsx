...
import { VertexServiceAccountFilePicker } from './vertex-service-account-file-picker'
...
export function ChannelMutateDrawer({
  ...
  const forcedRestoreKeyModeRef = useRef<'append' | null>(null)
  ...
  const hasExistingMultiKey = form.watch('has_existing_multi_key')
  const hasExistingVertexServiceAccountKey = form.watch(
  ...
  const vertexCredentialPresentation = useMemo(
    () =>
      getVertexCredentialPresentation({
        channelType: currentType,
        targetVertexKeyType: vertexKeyType || 'json',
        hasExistingMultiKey: hasExistingMultiKey === true,
  ...
  const isBatchMode =
    ...
    (currentType === 41 ||
      currentRow?.type === 41 ||
      hasExistingVertexServiceAccountKey === true)
      ? vertexCredentialPresentation.multiKeyMode !== 'single'
  ...
  useEffect(() => {
    if (isEditing && channelData?.data) {
      ...
      forcedRestoreKeyModeRef.current = null
      form.reset(defaults)
  ...
  useEffect(() => {
    const transition = getVertexCredentialSynchronization({
      isEditing,
      sourceChannelType: currentRow?.type,
      channelType: currentType,
      targetVertexKeyType: vertexKeyType || 'json',
      hasExistingMultiKey: hasExistingMultiKey === true,
      hasExistingVertexServiceAccountKey:
      ...
      multiKeyMode,
      keyMode,
      forcedRestoreKeyMode: forcedRestoreKeyModeRef.current,
    })
    if (!transition.shouldSynchronize) return

    forcedRestoreKeyModeRef.current = transition.forcedRestoreKeyMode ?? null

    if (multiKeyMode !== transition.multiKeyMode) {
      form.setValue('multi_key_mode', transition.multiKeyMode, {
        shouldDirty: true,
        shouldValidate: true,
      })
    }
    if (transition.keyMode && keyMode !== transition.keyMode) {
      form.setValue('key_mode', transition.keyMode, {
        shouldDirty: true,
        shouldValidate: true,
      })
    }
  ...
  const formPreviewFetcher = useCallback(
      ...
      const response = await fetchModels(
        {
          ...
          channel_id: editingAdvancedCustom
            ? channelId || undefined
            : undefined,
