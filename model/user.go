...
const userSettingSecretDomain = "users:setting"
...
type User struct {
	...
	AccessToken      *string                    `json:"-" gorm:"type:varchar(128);column:access_token;uniqueIndex"` // this token is for system management
	...
	Setting          string                     `json:"-" gorm:"-"`
	StoredSetting    string                     `json:"-" gorm:"type:text;column:setting"`
...
func (user *User) BeforeSave(_ *gorm.DB) error {
	stored, err := protectUserSettingValue(user.Setting)
	if err != nil {
		return err
	}
	user.StoredSetting = stored
	return nil
}

func (user *User) AfterFind(_ *gorm.DB) error {
	plaintext, err := openUserSettingValue(user.StoredSetting)
	if err != nil {
		return err
	}
	user.Setting = plaintext
	return nil
}

func inspectUserSettingValue(plaintext string) (bool, error) {
	if plaintext == "" {
		return false, nil
	}
	var setting dto.UserSetting
	if err := common.Unmarshal([]byte(plaintext), &setting); err != nil {
		return false, errors.New("invalid user setting JSON")
	}
	return setting.WebhookSecret != "" || setting.GotifyToken != "", nil
}

func protectUserSettingValue(plaintext string) (string, error) {
	...
	return common.SealDataEncryptionValue(userSettingSecretDomain, plaintext)
}

func userSettingCacheValue(plaintext string) (string, bool, error) {
	protected, err := inspectUserSettingValue(plaintext)
	if err != nil {
		return "", false, err
	}
	...
	if !common.DataEncryptionConfigured() {
		return "", false, nil
	}
...
func openUserSettingValue(stored string) (string, error) {
	if stored == "" {
		return "", nil
	}
	if !common.IsDataEncryptionEnvelope(stored) {
		protected, err := inspectUserSettingValue(stored)
		if err != nil {
			return "", err
		}
		if protected && common.DataEncryptionEnabled() {
			return "", errors.New(
				"legacy plaintext remains in protected user setting while encryption enforcement is enabled",
			)
		}
		return stored, nil
	}
...
func UpdateUserSetting(userId int, setting dto.UserSetting) error {
	...
	storedSetting, err := protectUserSettingValue(settingValue)
	if err != nil {
		return err
	}
	if err = DB.Model(&User{}).Where("id = ?", userId).Update("setting", storedSetting).Error; err != nil {
		return err
	}
...
func BindEmailToUser(user *User, email string) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return BindEmailToUserWithTx(tx, user, email)
	}); err != nil {
		return err
	}
...
func BindEmailToUserWithTx(tx *gorm.DB, user *User, email string) error {
...
func BindEmailToUserWithVerification(user *User, email, token string) error {
	if user == nil || user.Id <= 0 {
		return gorm.ErrInvalidData
	}
	if err := ConsumeEmailVerificationFlowWithAction(token, email, func(tx *gorm.DB) error {
		return BindEmailToUserWithTx(tx, user, email)
	}); err != nil {
		return err
	}
	if err := updateUserCache(*user); err != nil {
		common.SysError(fmt.Sprintf("failed to publish email binding metadata for user %d: %v", user.Id, err))
	}
	return nil
}
...
func (user *User) finishInsert(inviterId int) {
	...
	if err := DB.First(&createdUser, user.Id).Error; err == nil {
		// 生成基于角色的默认边栏配置
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	newUser := *user
	newUser.StoredSetting, err = protectUserSettingValue(newUser.Setting)
	if err != nil {
		return err
	}
	current := User{}
...
// BindBuiltInExternalIdentityWithAuthVersion updates a built-in login
// identity and advances the user's authentication version in one transaction.
// The caller is responsible for preserving its verified session or revoking
// all target sessions after the commit.
func BindBuiltInExternalIdentityWithAuthVersion(userID int, provider, subject string) (*User, error) {
	provider = strings.ToLower(strings.TrimSpace(provider))
	subject = strings.TrimSpace(subject)
	column, ok := builtInAuthenticationBindingColumns[provider]
	if userID <= 0 || !ok || subject == "" {
		return nil, errors.New("invalid external identity binding")
	}
	var updated User
	if err := DB.Transaction(func(tx *gorm.DB) error {
		if err := ReplaceExternalIdentityWithTx(tx, provider, subject, userID); err != nil {
			return err
		}
		if _, err := IncrementUserAuthVersionWithTx(tx, userID); err != nil {
			return err
		}
		if err := tx.Model(&User{}).Where("id = ?", userID).Update(column, subject).Error; err != nil {
			return err
		}
...
func (user *User) ClearBinding(bindingType string) error {
	...
	for provider, column := range builtInAuthenticationBindingColumns {
		bindingColumnMap[provider] = column
	}
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if _, ok := builtInAuthenticationBindingColumns[bindingType]; ok {
...
// Delete soft-deletes the user row so audit and billing history keep resolving
// the account, but purges the authentication surface outright. Secrets have no
// restore path — the codebase has no undelete — so a deleted account must not
// leave a usable API key, TOTP secret, passkey, or external identity binding
// behind. Hard deletion runs the same purge, which keeps the two paths
// consistent and makes cleanup idempotent when a delete is retried.
...
func (user *User) ValidateAndFill() (err error) {
	...
	// The identifier may be a username or an email address. Usernames are
	// stored verbatim and stay case-sensitive; emails are normalized to
	// lowercase on write, so they must be matched that way here too --
	// PostgreSQL and SQLite compare text case-sensitively, so a plain
	// equality test locked mixed-case addresses out on those databases while
	// MySQL's default collation let them in.
	err = DB.Where("username = ? OR LOWER(email) = ?", username, NormalizeEmail(username)).First(user).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			// Burn one bcrypt comparison so a missing account is not
			// distinguishable from a wrong password by response time.
			common.BurnPasswordCheckTime(password)
...
func (user *User) FillUserByEmail() error {
	...
	err := DB.Where("LOWER(email) = ?", NormalizeEmail(user.Email)).First(user).Error
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return err
	}
	return nil
...
func (user *User) UpdateGitHubId(newGitHubId string) error {
	...
	newGitHubId = strings.TrimSpace(newGitHubId)
	if newGitHubId == "" {
		return errors.New("GitHub id is empty")
	}
	if err := DB.Transaction(func(tx *gorm.DB) error {
		if err := ReplaceExternalIdentityWithTx(
			tx,
		...
		return tx.Model(user).Update("github_id", newGitHubId).Error
	}); err != nil {
		return err
	}
...
func ValidateAccessToken(token string) (*User, error) {
	return ValidateDashboardAccessToken(token)
}

// GetUserQuota reads the authorization balance from the database. The fromDB
// argument remains for source compatibility; Redis stores user metadata only.
func GetUserQuota(id int, _ bool) (int, error) {
...
func GetUserSetting(id int, fromDB bool) (settingMap dto.UserSetting, err error) {
	...
	if safeSetting.Valid {
		setting, err = openUserSettingValue(safeSetting.String)
		if err != nil {
			return settingMap, err
		}
	} else {
