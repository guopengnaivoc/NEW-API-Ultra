...
func Query(params QueryParams) (QueryResult, error) {
	...
	// The Redis active bucket aggregates the current bucket across all
	// nodes (each node writes its samples there as well as to its local
	// hot bucket). When it is merged, the local hot bucket for that same
	// key must be skipped or this node's samples would count twice.
