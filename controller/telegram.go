...
var (
	...
	errTelegramBindSessionIdle      = errors.New("telegram bind session reached its idle deadline")
)

type telegramBindCommit struct {
	FlowToken          string
	UserID             int
	SessionID          string
	TelegramID         string
...
func commitTelegramBindAt(input telegramBindCommit, now time.Time) error {
	_, err := model.ConsumeAuthFlowWithActionAt(input.FlowToken, model.AuthFlowMatch{
		Purpose:   model.AuthFlowPurposeTelegramBind,
		UserId:    input.UserID,
		SessionId: input.SessionID,
	}, now, func(tx *gorm.DB, flow *model.AuthFlow) error {
		if err := model.ClaimExternalAuthAssertionWithTxAt(
			tx,
		...
		nowUnix := now.Unix()
		if session.Status != model.UserSessionStatusActive || session.RevokedAt != 0 || session.ExpiresAt <= nowUnix {
			return service.ErrLoginSessionRevoked
		}
		if !session.IsActiveAt(nowUnix) {
			return errTelegramBindSessionIdle
		}
	...
	if !errors.Is(err, errTelegramBindSessionIdle) {
		return err
	}

	// The binding transaction has rolled back. Reconcile the idle observation
	// through the centralized authority so the revocation remains durable. If a
	// concurrent refresh won, its newer activity survives, but this callback
	// still fails closed because it observed the session at the idle deadline.
	_, reconcileErr := model.GetActiveUserSessionBySIDAt(input.SessionID, now.Unix())
	if reconcileErr != nil && !errors.Is(reconcileErr, model.ErrUserSessionInactive) {
		return reconcileErr
	}
	return service.ErrLoginSessionRevoked
}
...
func TelegramBind(c *gin.Context) {
	...
	if err := model.PublishUserAuthCache(pendingFlow.UserId); err != nil {
		common.SysError("TelegramBind user cache publish failed: " + err.Error())
		telegramBindFailure(c, telegramBindErrorInternal)
		return
	}
	if _, err := service.AdvanceCurrentSessionToUserVersion(identity, "telegram_bound"); err != nil {
		common.SysError("TelegramBind session advance failed: " + err.Error())
		telegramBindFailure(c, telegramBindErrorSessionInvalid)
		return
	}
