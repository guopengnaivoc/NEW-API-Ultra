...
import { createResponseRenderer } from './response-renderer'
...
export const Response = memo((props: ResponseProps) => {
  ...
  const renderedContent =
    parsedContent.bodyNodes.length > 0
      ? renderer.renderChildren(parsedContent.bodyNodes)
      : content
  const footnotes = renderer.renderFootnotes(parsedContent.footnotes)
