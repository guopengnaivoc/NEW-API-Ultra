	...
	if model.IsWeChatIdAlreadyTaken(wechatId) {
		...
		if common.RegisterEnabled {
			...
			if err != nil {
				...
				if fillErr := existing.FillUserByWeChatId(); fillErr != nil || existing.Id == 0 {
					c.JSON(http.StatusOK, gin.H{
						"success": false,
						"message": err.Error(),
					})
...
func WeChatBind(c *gin.Context) {
	...
	updated, err := model.BindBuiltInExternalIdentityWithAuthVersion(
		user.Id,
		model.ExternalIdentityProviderWeChat,
		wechatId,
	)
	...
	bundle, err := service.AdvanceCurrentSessionToUserVersion(identity, "wechat_bound")
	if err != nil {
		common.ApiError(c, err)
		return
	}
	recordUserSecurityAudit(c, updated.Id, "user.external_identity_bind", map[string]interface{}{
		"provider": "wechat",
	})
	setAuthNoStore(c)
	c.JSON(http.StatusOK, gin.H{
