...
func fetchJSON[T any](ctx context.Context, url string, out *upstreamEnvelope[T]) error {
	if err := checkUpstreamMetadataSource(url); err != nil {
		return err
	}
	var lastErr error
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		func() {
			...
			case http.StatusOK:
				...
				// Verified before the body is cached or decoded, so a
				// tampered document neither reaches the model catalogue nor
				// gets served to a later 304 from the cache.
				if err := checkUpstreamMetadataBody(url, buf); err != nil {
					lastErr = err
					return
				}
				// cache body and ETag
