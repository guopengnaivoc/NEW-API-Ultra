...
func applyOperations(jsonData []byte, operations []ParamOperation, conditionContext map[string]interface{}) ([]byte, error) {
	...
	sanitizeRequestHeadersContext(context)
...
func getHeaderValueFromContext(context map[string]interface{}, headerName string) (string, bool) {
	...
	for _, key := range []string{paramOverrideContextHeaderOverride, paramOverrideContextRequestHeaders} {
		if key == paramOverrideContextRequestHeaders && IsSensitiveClientHeader(headerName) {
			continue
		}
...
func buildRequestHeadersContext(headers map[string]string) map[string]interface{} {
	...
	normalizedEntries := lo.FilterMap(entries, func(item lo.Entry[string, string], _ int) (lo.Entry[string, string], bool) {
		...
		if normalized == "" || value == "" || IsSensitiveClientHeader(normalized) {
...
func sanitizeRequestHeadersContext(context map[string]interface{}) {
	if context == nil {
		return
	}
	raw, exists := context[paramOverrideContextRequestHeaders]
	if !exists {
		return
	}
	...
	case map[string]string:
		context[paramOverrideContextRequestHeaders] = buildRequestHeadersContext(headers)
	case map[string]interface{}:
		...
		context[paramOverrideContextRequestHeaders] = filtered
	default:
		context[paramOverrideContextRequestHeaders] = map[string]interface{}{}
	}
}
