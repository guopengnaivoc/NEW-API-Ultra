...
func renderNotifyContent(data dto.Notify) (string, error) {
	parts := strings.Split(data.Content, dto.ContentValueParam)
	placeholderCount := len(parts) - 1
	if placeholderCount != len(data.Values) {
		return "", fmt.Errorf(
			"notification content has %d placeholders but %d values",
			placeholderCount,
			len(data.Values),
		)
	}
	...
	content.Grow(len(data.Content))
	for i, part := range parts {
		content.WriteString(part)
		if i < len(data.Values) {
			content.WriteString(fmt.Sprintf("%v", data.Values[i]))
		}
	}
...
func sendEmailNotify(userEmail string, data dto.Notify) error {
	content, err := renderNotifyContent(data)
	if err != nil {
		return err
	}
