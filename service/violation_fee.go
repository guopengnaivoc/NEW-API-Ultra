...
func calcViolationFeeQuota(amount, groupRatio float64) (int, error) {
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.NewAPIError) bool {
	...
	feeQuota, err := calcViolationFeeQuota(settings.ViolationDeductionAmount, groupRatio)
	if err != nil {
		logger.LogError(ctx, fmt.Sprintf("failed to calculate violation fee: %s", err.Error()))
		return false
	}
	if feeQuota <= 0 {
		return false
