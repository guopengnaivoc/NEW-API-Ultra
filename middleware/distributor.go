...
func Distribute() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if ok {
			...
			if modelLimitEnable && !isModelLessTaskLookupRoute(c) {
				...
				if modelRequest.Model == "" {
					// Fail closed: an enforced route whose effective model could not be
					// resolved must never fall through unchecked.
					abortWithOpenAiMessage(c, http.StatusForbidden, i18n.T(c, i18n.MsgDistributorTokenNoModelAccess))
					return
				}
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.Contains(c.Request.URL.Path, "/mj/") {
		...
		c.Set("relay_mode", relayMode)
		shouldSelectChannel = false
		// remix 复用原任务的模型进行提交与计费，因此授权必须校验该模型，
		// 而不是校验一个空模型名。
		modelRequest.Model = getRemixOriginModelName(c)
...
func getTaskOriginModelName(c *gin.Context) string {
	...
	if task, exist, err := model.GetByTaskId(userId, taskId); err == nil && exist && task != nil {
		return task.EffectiveModelName()
	}
	return ""
}

// getRemixOriginModelName 回填 POST /v1/videos/:video_id/remix 的生效模型。
// remix 是计费提交，其模型来自原任务（可能由同一用户的另一个 token 创建），
// 因此必须在计费、选取渠道与上游提交之前拿到该模型并对请求 token 的
// 「可用模型限制」做校验。解析失败时返回空串，由调用方按 fail-closed 处理。
func getRemixOriginModelName(c *gin.Context) string {
	if !common.GetContextKeyBool(c, constant.ContextKeyTokenModelLimitEnabled) {
		return ""
	}
	...
	task, exist, err := model.GetByTaskId(c.GetInt("id"), videoId)
	if err != nil || !exist {
		return ""
	}
	return task.EffectiveModelName()
...
// isModelLessTaskLookupRoute 标识本身就不携带模型、且只读取已有任务状态的路由。
// 这些路由不计费、不选取渠道、不向上游提交新作业，因此在 token 启用
// 「可用模型限制」时豁免模型校验。豁免必须按路由身份判定，绝不能退化为
// 「模型名为空即放行」——那会把 remix 等计费提交一并放过。
func isModelLessTaskLookupRoute(c *gin.Context) bool {
	path := c.Request.URL.Path
	if strings.Contains(path, "/mj/") {
		switch relayconstant.Path2RelayModeMidjourney(path) {
	...
	if strings.Contains(path, "/suno/") {
		switch relayconstant.Path2RelaySuno(c.Request.Method, path) {
