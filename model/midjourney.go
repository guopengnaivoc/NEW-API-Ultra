...
type Midjourney struct {
	...
	QuotaReservationId int    `json:"-" gorm:"index"`
...
func (midjourney *Midjourney) UpdateWithQuotaReservation(
	fromStatus string,
	outcome MidjourneyBillingOutcome,
) (bool, bool, error) {
	if midjourney == nil || midjourney.Id <= 0 || !validMidjourneyBillingOutcome(outcome) {
		return false, false, ErrMidjourneyQuotaReservationInvalid
	}
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		if midjourney.QuotaReservationId > 0 {
			if err := lockForUpdate(tx).
				Where("id = ?", midjourney.QuotaReservationId).
				First(&reservation).Error; err != nil {
				return err
			}
			if reservation.UserId != midjourney.UserId ||
				reservation.Quota != midjourney.Quota ||
				reservation.MidjourneyTaskId != midjourney.Id {
		...
		} else if outcome == MidjourneyBillingOutcomeFailure && midjourney.Quota > 0 {
			userUpdate := tx.Unscoped().Model(&User{}).
				Where("id = ?", midjourney.UserId).
				Update("quota", gorm.Expr("quota + ?", midjourney.Quota))
			if userUpdate.Error != nil {
				return userUpdate.Error
			}
			if userUpdate.RowsAffected != 1 {
				return ErrMidjourneyQuotaReservationInvalid
			}
			refunded = true
		}
	...
	if refunded {
		...
		} else if err := invalidateUserCache(midjourney.UserId); err != nil {
			common.SysError("failed to invalidate legacy Midjourney quota user cache: " + err.Error())
		}
	}
	return won, refunded, nil
}
