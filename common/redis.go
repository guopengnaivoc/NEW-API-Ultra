...
var (
	ErrRedisKeyMissing    = errors.New("redis mutation target is missing")
	ErrRedisKeyPersistent = errors.New("redis mutation target has no expiration")
)
...
func redisMutateExpiringKey(key string, operation string, args ...interface{}) error {
	const script = `
...
return ttl
`
	arguments := make([]interface{}, 0, len(args)+1)
	arguments = append(arguments, operation)
	arguments = append(arguments, args...)
	result, err := RDB.Eval(context.Background(), script, []string{key}, arguments...).Int64()
	if err != nil {
		return fmt.Errorf("redis %s mutation failed for %q: %w", operation, key, err)
	}
	switch result {
	case -2:
		return fmt.Errorf("redis %s mutation failed for %q: %w", operation, key, ErrRedisKeyMissing)
	case -1:
		return fmt.Errorf("redis %s mutation failed for %q: %w", operation, key, ErrRedisKeyPersistent)
	default:
		return nil
	}
}

// RedisIncr atomically increments an expiring string without renewing its TTL.
func RedisIncr(key string, delta int64) error {
	if DebugEnabled {
		SysLog(fmt.Sprintf("Redis INCR: key=%s, delta=%d", key, delta))
	}
	return redisMutateExpiringKey(key, "incr", delta)
}
...
func RedisHIncrBy(key, field string, delta int64) error {
	...
	return redisMutateExpiringKey(key, "hincr", field, delta)
}
...
func RedisHSetField(key, field string, value interface{}) error {
	...
	return redisMutateExpiringKey(key, "hset", field, value)
}
