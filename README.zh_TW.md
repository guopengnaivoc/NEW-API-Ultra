...
> 為保留歸屬資訊，原始碼儲存庫仍列為 [`QuantumNous/new-api`](https://github.com/QuantumNous/new-api)，其歷史映像為 `calciumion/new-api:latest`。上游的複製/映像指令不是本發布快照的部署路徑。

> 本倉庫是穩定化發布快照，不代表所有未處理問題都已解決。對外提供服務前，請固定明確版本並完成部署驗證。
...
預建置映像或單容器用法請參閱 [`DEPLOYMENT.md`](./DEPLOYMENT.md)。建議使用上面的 Compose 流程，因為它會設定 PostgreSQL、Redis、穩定的工作階段/加密金鑰和資料加密金鑰環。不要在缺少這些設定和持久化掛載時直接複製裸 `docker run` 命令。

</details>

---

🎉 部署完成後，訪問 `http://127.0.0.1:3000` 即可使用！
...
> [!TIP]
> **帶版本 Docker 映像（完成發布流程後提供）：** `ghcr.io/guopengnaivoc/new-api-ultra:v0.1.0-main.49270e59-r1`
...
| `DATA_ENCRYPTION_KEYS` | 可逆敏感資訊使用的根密鑰環，格式為逗號分隔的 `key-id=base64-key`；每個解碼後的密鑰必須正好 32 位元組，所有節點必須一致 | - |
| `DATA_ENCRYPTION_ACTIVE_KEY_ID` | 新加密信封與根密鑰重新包裹使用的密鑰識別碼 | - |
| `DATA_ENCRYPTION_ENABLE` | 靜態加密強制開關與遷移閘門；僅可在文件說明的混合版本準備階段暫時關閉 | `true` |
| `SQL_DSN` | 資料庫連接字符串                                                     | - |
...
1. 安裝寶塔面板（≥ 9.2.0 版本）
2. 請依照本儲存庫的 [`DEPLOYMENT.md`](./DEPLOYMENT.md) 或
   [`寶塔教學`](./docs/installation/BT.md) 部署：複製 `NEW-API-Ultra`、產生 `.env`，
   然後啟動本儲存庫的 Docker Compose 服務。
3. 寶塔應用商店中的 **New-API** 一鍵安裝套件與上游映像僅供參考，**不是本快照的部署路徑**。
...
**[部署指南](./DEPLOYMENT.md)** • **[本倉庫 Issues](https://github.com/guopengnaivoc/NEW-API-Ultra/issues)** • **[本倉庫 Releases](https://github.com/guopengnaivoc/NEW-API-Ultra/releases)** • **[上游文件](https://docs.newapi.pro/zh/docs)** • **[上游 Issues](https://github.com/Calcium-Ion/new-api/issues)** • **[上游發布](https://github.com/Calcium-Ion/new-api/releases)**
