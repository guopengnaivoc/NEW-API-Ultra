...
> 为保留归属信息，原始源码仓库仍列为 [`QuantumNous/new-api`](https://github.com/QuantumNous/new-api)，其历史镜像为 `calciumion/new-api:latest`。上游的克隆/镜像命令不是本发布快照的部署路径。

## 📝 项目说明

> 本仓库是一个稳定化发布快照，不代表所有未处理问题都已解决。对外提供服务前，请使用明确版本并完成部署验证。
...
预构建镜像或单容器用法请参阅 [`DEPLOYMENT.md`](./DEPLOYMENT.md)。推荐使用上面的 Compose 流程，因为它会配置 PostgreSQL、Redis、稳定的会话/加密密钥和数据加密密钥环。不要在缺少这些设置和持久化挂载时直接复制裸 `docker run` 命令。

</details>

---

🎉 部署完成后，访问 `http://127.0.0.1:3000` 即可使用！
...
| `DATA_ENCRYPTION_KEYS` | 可逆敏感信息使用的根密钥环，格式为逗号分隔的 `key-id=base64-key`；每个解码后的密钥必须正好 32 字节，所有节点必须一致 | - |
| `DATA_ENCRYPTION_ACTIVE_KEY_ID` | 新加密信封和根密钥重包裹使用的密钥标识 | - |
| `DATA_ENCRYPTION_ENABLE` | 静态加密强制开关和迁移闸门；仅可在文档说明的混合版本准备阶段临时关闭 | `true` |
| `SQL_DSN` | 数据库连接字符串                                                     | - |
...
1. 安装宝塔面板（≥ 9.2.0 版本）
2. 请按照本仓库的 [`DEPLOYMENT.md`](./DEPLOYMENT.md) 或
   [`宝塔教程`](./docs/installation/BT.md) 部署：克隆 `NEW-API-Ultra`，生成 `.env`，
   然后启动本仓库的 Docker Compose 服务。
3. 宝塔应用商店中的 **New-API** 一键安装包和上游镜像仅供参考，**不是本快照的部署路径**。
...
**[部署指南](./DEPLOYMENT.md)** • **[本仓库 Issues](https://github.com/guopengnaivoc/NEW-API-Ultra/issues)** • **[本仓库 Releases](https://github.com/guopengnaivoc/NEW-API-Ultra/releases)** • **[上游文档](https://docs.newapi.pro/zh/docs)** • **[上游 Issues](https://github.com/Calcium-Ion/new-api/issues)** • **[上游发布](https://github.com/Calcium-Ion/new-api/releases)**
