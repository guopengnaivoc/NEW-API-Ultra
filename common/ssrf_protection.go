...
type SSRFProtection struct {
	...
	AllowedPorts           []PortRange // 允许的端口范围
...
// PortRange 表示一段闭区间端口范围；单个端口以 Start == End 表示。
...
// 支持格式: "80", "443", "8000-9000"。范围保持区间表示，
// 不再展开成逐个整数（"1-65535" 曾导致每次校验分配 64Ki 个 int）。
func parsePortRanges(portConfigs []string) ([]PortRange, error) {
	var ports []PortRange
	...
	for _, config := range portConfigs {
		...
		if strings.Contains(config, "-") {
			...
			ports = append(ports, PortRange{Start: startPort, End: endPort})
		} else {
			// 处理单个端口 "80"
...
func (p *SSRFProtection) isAllowedPort(port int) bool {
	...
	for _, allowedRange := range p.AllowedPorts {
		if port >= allowedRange.Start && port <= allowedRange.End {
			return true
		}
	}
	return false
