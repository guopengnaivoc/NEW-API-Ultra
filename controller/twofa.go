...
func Setup2FA(c *gin.Context) {
	...
	if err := twoFA.CreatePendingTwoFASetupWithBackupCodes(backupCodes); err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "保存2FA设置失败",
		})
		common.SysLog("保存2FA设置失败: " + err.Error())
		return
	}
...
func Enable2FA(c *gin.Context) {
	...
	expired, err := twoFA.ExpirePendingSetup(time.Now())
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	// 走计数器占用路径，确保确认设置消耗的验证码不能在同一时间步
	// 被重放到后续登录/敏感操作验证。
	validSetupCode, err := twoFA.ValidateTOTPAndUpdateUsage(cleanCode)
	if err != nil {
		common.ApiError(c, err)
		return
	}
	...
	if err := twoFA.EnableWithAuthVersion(); err != nil {
		if errors.Is(err, model.ErrTwoFASetupExpired) {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "2FA设置已过期，请重新初始化设置",
			})
...
func Get2FAStatus(c *gin.Context) {
	...
	now := time.Now()
	if twoFA != nil && twoFA.IsPendingSetupExpired(now) {
		expired, err := twoFA.ExpirePendingSetup(now)
		if err != nil {
			common.ApiError(c, err)
			return
		}
