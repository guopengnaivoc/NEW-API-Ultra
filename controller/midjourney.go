...
func runMidjourneyTaskUpdateOnce(ctx context.Context, report func(processed, total int)) midjourneyPollSummary {
	...
	if len(nullTasks) > 0 {
		...
		for _, task := range nullTasks {
			...
			task.Status = "FAILURE"
			task.Progress = "100%"
			task.FailReason = "Midjourney task has empty mj_id"
			won, refunded, err := task.UpdateWithQuotaReservation(
				preStatus,
				model.MidjourneyBillingOutcomeFailure,
			)
			if err != nil {
			...
			if refunded {
				recordMidjourneyRefund(ctx, task, "任务 ID 为空")
			}
		}
	}
	...
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			...
			for _, taskID := range taskIds {
				...
				task.Status = "FAILURE"
				task.Progress = "100%"
				_, refunded, updateErr := task.UpdateWithQuotaReservation(
					preStatus,
					model.MidjourneyBillingOutcomeFailure,
				)
				if updateErr != nil {
					logger.LogError(ctx, fmt.Sprintf("UpdateMidjourneyTask #%d error: %v", task.Id, updateErr))
					continue
				}
				if refunded {
					recordMidjourneyRefund(ctx, task, "获取渠道信息失败")
				}
			}
			continue
		}
		...
		responseBody, err := service.ReadMidjourneyResponseBody(resp)
		if err != nil {
		...
		if err != nil {
			logger.LogError(ctx, fmt.Sprintf("Get Mjp Task parse body error2: %v", err))
			resp.Body.Close()
			cancel()
			continue
		}
		...
		for _, responseItem := range responseItems {
			...
			_, refunded, err := task.UpdateWithQuotaReservation(preStatus, outcome)
			if err != nil {
				logger.LogError(ctx, "UpdateMidjourneyTask task error: "+err.Error())
			} else if refunded {
				recordMidjourneyRefund(ctx, task, "构图失败")
			}
		}
	}
