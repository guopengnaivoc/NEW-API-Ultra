...
func (s *fakeSMTPServer) serve() {
	...
	for {
		...
		case upperCommand == "QUIT":
			if s.quitError {
				_ = writeSMTPLine(rw, "421 4.3.0 forced quit failure")
				return
			}
...
func TestParseSingleEmailAddress(t *testing.T) {
	tests := []struct {
		name    string
		input   string
		want    string
		...
		{name: "plain", input: "user@example.com", want: "user@example.com"},
		{name: "display name", input: "Example User <user@example.com>", want: "user@example.com"},
		{name: "semicolon fanout", input: "victim@example.com;attacker@example.com", wantErr: true},
		{name: "comma list", input: "victim@example.com, attacker@example.com", wantErr: true},
		{name: "crlf", input: "victim@example.com\r\nBcc: attacker@example.com", wantErr: true},
		{name: "empty", input: "", wantErr: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := ParseSingleEmailAddress(test.input)
			if test.wantErr {
				require.Error(t, err)
				assert.Empty(t, got)
				return
...
func TestSendEmailRejectsRecipientFanoutBeforeData(t *testing.T) {
	...
	case message := <-server.messages:
		t.Fatalf("unexpected SMTP DATA: %s", message)
	default:
	}
}
...
func TestSendEmailQuitErrorLogOmitsRecipient(t *testing.T) {
	server := newFakeSMTPServerWithSTARTTLSAdvertisement(t, false)
	server.quitError = true
	defer server.close()
	withSMTPSettings(t)
	...
	commonErrorWriter := gin.DefaultErrorWriter
	LogWriterMu.Lock()
	gin.DefaultErrorWriter = &logOutput
	LogWriterMu.Unlock()
	t.Cleanup(func() {
		LogWriterMu.Lock()
		gin.DefaultErrorWriter = commonErrorWriter
		LogWriterMu.Unlock()
	})

	recipient := "quit-recipient@example.com"
	err := SendEmail("Verification", recipient, "<p>secret</p>")

	require.Error(t, err)
	assert.Contains(t, logOutput.String(), "SMTP")
	assert.NotContains(t, logOutput.String(), recipient)
}
