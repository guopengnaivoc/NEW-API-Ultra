...
export function AccessTokenDialog({
  ...
  const { token, expiresAt, generating, generate, clear } = useAccessToken()
  const {
  ...
  const handleGenerate = useCallback(async () => {
    await withVerification((proofToken) => generate(proofToken), {
      scope: 'pat.rotate',
      target: 'self',
      ...
      >
        <div className='my-6 space-y-4'>
          <div className='space-y-2'>
            ...
            {expiresAt ? (
              <p className='text-muted-foreground text-xs'>
                {t('Expires at')}: {formatTimestampToDate(expiresAt)}
              </p>
            ) : null}
          </div>
        </div>
