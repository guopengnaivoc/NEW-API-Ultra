...
// UpdateUserOAuthBindingWithAuthVersion mutates a custom OAuth login identity
// and advances auth_version in the same transaction.
func UpdateUserOAuthBindingWithAuthVersion(userId, providerId int, newProviderUserId string) error {
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		...
		var binding UserOAuthBinding
		err := tx.Where("user_id = ? AND provider_id = ?", userId, providerId).
			First(&binding).Error
		...
		return tx.Model(&binding).
			Update("provider_user_id", newProviderUserId).Error
	}); err != nil {
		return err
	}
...
// DeleteUserOAuthBindingWithAuthVersion removes a custom OAuth login identity
// and advances auth_version in the same transaction.
func DeleteUserOAuthBindingWithAuthVersion(userId, providerId int) error {
