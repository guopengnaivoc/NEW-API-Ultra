...
func TestQuotaFromDecimalStrictReturnsTypedClampError(t *testing.T) {
	quota, err := QuotaFromDecimalStrict(decimal.NewFromFloat(42.5))
	require.NoError(t, err)
	assert.Equal(t, 43, quota)

	quota, err = QuotaFromDecimalStrict(
		decimal.NewFromInt(int64(MaxQuota)).Add(decimal.NewFromInt(1)),
	)
	assert.Zero(t, quota)
	var clamp *QuotaClamp
	require.ErrorAs(t, err, &clamp)
	...
	quota, err = QuotaFromDecimalStrict(
		decimal.NewFromInt(int64(MinQuota)).Sub(decimal.NewFromInt(1)),
	)
	assert.Zero(t, quota)
	require.ErrorAs(t, err, &clamp)
	assert.Equal(t, QuotaClampUnderflow, clamp.Kind)
