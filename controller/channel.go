...
func FetchUpstreamModels(c *gin.Context) {
	...
	ids, err := fetchChannelUpstreamModelIDsWithContext(
		c.Request.Context(),
		channel,
	)
	if err != nil {
...
// 此函数依赖 ChannelKeySecurityProofRequired 中间件，确保用户已通过安全验证
func GetChannelKey(c *gin.Context) {
	...
	channelId, err := strconv.Atoi(target)
	if err != nil {
		common.ApiError(c, fmt.Errorf("渠道ID格式错误: %v", err))
		return
	}
...
func RefreshCodexChannelCredential(c *gin.Context) {
	...
	result, err := service.RefreshCodexChannelCredential(
		ctx,
		channelId,
...
const (
	vertexServiceAccountMaxKeys       = 100
	vertexServiceAccountMaxKeyBytes   = 64 * 1024
	vertexServiceAccountMaxTotalBytes = 1024 * 1024
)

func parseChannelOtherSettings(raw string) (dto.ChannelOtherSettings, error) {
	settings := dto.ChannelOtherSettings{}
	if raw == "" {
		return settings, nil
	}
	if common.GetJsonType(json.RawMessage(raw)) != "object" {
		return settings, fmt.Errorf("channel settings must be a JSON object")
	}
...
func isVertexServiceAccountChannel(
	channelType int,
	settings dto.ChannelOtherSettings,
) bool {
	return channelType == constant.ChannelTypeVertexAi &&
		settings.VertexKeyType != dto.VertexKeyTypeAPIKey
}

func normalizeVertexServiceAccountKey(raw []byte) (string, error) {
	if len(raw) > vertexServiceAccountMaxKeyBytes {
		return "", fmt.Errorf("Vertex AI service account key exceeds the 64 KiB limit")
	}

	var object map[string]json.RawMessage
	if err := common.Unmarshal(raw, &object); err != nil || object == nil {
		return "", fmt.Errorf("Vertex AI service account key must be a valid JSON object")
	}

	requiredFields := []string{"project_id", "private_key", "client_email"}
	for _, field := range requiredFields {
		rawValue, ok := object[field]
		if !ok {
			return "", fmt.Errorf("Vertex AI service account key must include a non-empty %s", field)
		}
		var value string
		if err := common.Unmarshal(rawValue, &value); err != nil || strings.TrimSpace(value) == "" {
			return "", fmt.Errorf("Vertex AI service account key must include a non-empty %s", field)
		}
	}
	...
	if rawType, ok := object["type"]; ok {
		_ = common.Unmarshal(rawType, &keyType)
	}
	if keyType != "service_account" {
		return "", fmt.Errorf("Vertex AI service account key type must be service_account")
	}

	normalized, err := common.Marshal(object)
	if err != nil {
		return "", fmt.Errorf("failed to normalize Vertex AI service account key")
	}
	if len(normalized) > vertexServiceAccountMaxKeyBytes {
		return "", fmt.Errorf("Vertex AI service account key exceeds the 64 KiB limit")
	}
	return string(normalized), nil
}

func validateVertexServiceAccountCollection(keys []string) error {
	if len(keys) == 0 {
		return fmt.Errorf("Vertex AI service account keys cannot be empty")
	}
	if len(keys) > vertexServiceAccountMaxKeys {
		return fmt.Errorf("Vertex AI service account keys exceed the 100-key limit")
	}

	totalBytes := 0
	for _, key := range keys {
		if len(key) > vertexServiceAccountMaxKeyBytes {
			return fmt.Errorf("Vertex AI service account key exceeds the 64 KiB limit")
		}
		totalBytes += len(key)
		if totalBytes > vertexServiceAccountMaxTotalBytes {
			return fmt.Errorf("Vertex AI service account keys exceed the 1 MiB total limit")
		}
	}
	return nil
}

func parseVertexServiceAccountKeys(keys string, arrayInput bool) ([]string, error) {
	if keys == "" {
		return nil, fmt.Errorf("Vertex AI service account keys cannot be empty")
	}
	if len(keys) > vertexServiceAccountMaxTotalBytes {
		return nil, fmt.Errorf("Vertex AI service account keys exceed the 1 MiB total limit")
	}

	if !arrayInput {
		key, err := normalizeVertexServiceAccountKey([]byte(keys))
		if err != nil {
			return nil, err
		}
	...
	if err := common.Unmarshal([]byte(keys), &rawKeys); err != nil {
		return nil, fmt.Errorf("Vertex AI batch keys must be a JSON array")
	}
	if len(rawKeys) == 0 {
		return nil, fmt.Errorf("Vertex AI service account keys cannot be empty")
	}
	if len(rawKeys) > vertexServiceAccountMaxKeys {
		return nil, fmt.Errorf("Vertex AI service account keys exceed the 100-key limit")
	}

	normalized := make([]string, 0, len(rawKeys))
	for _, rawKey := range rawKeys {
		key, err := normalizeVertexServiceAccountKey(rawKey)
		if err != nil {
			return nil, err
		}
		normalized = append(normalized, key)
	}
	if err := validateVertexServiceAccountCollection(normalized); err != nil {
		return nil, err
	}
...
func getVertexArrayKeys(keys string) ([]string, error) {
	return parseVertexServiceAccountKeys(keys, true)
}

func parseStoredVertexServiceAccountKeys(keys string) ([]string, error) {
	...
	if strings.Count(trimmed, "\n")+1 > vertexServiceAccountMaxKeys {
		return nil, fmt.Errorf("Vertex AI service account keys exceed the 100-key limit")
	}

	rawKeys := strings.Split(strings.Trim(trimmed, "\n"), "\n")
	normalized := make([]string, 0, len(rawKeys))
	for _, rawKey := range rawKeys {
		key, err := normalizeVertexServiceAccountKey([]byte(strings.TrimSpace(rawKey)))
		if err != nil {
			return nil, err
		}
		normalized = append(normalized, key)
...
func parseVertexServiceAccountUpdateKeys(keys string, allowMany bool) ([]string, error) {
	if allowMany && strings.HasPrefix(strings.TrimSpace(keys), "[") {
		return getVertexArrayKeys(keys)
	}
	return parseVertexServiceAccountKeys(keys, false)
}

func appendUniqueVertexServiceAccountKeys(existingKeys, newKeys []string) []string {
	seen := make(map[string]struct{}, len(existingKeys)+len(newKeys))
	combined := append([]string(nil), existingKeys...)
...
func AddChannel(c *gin.Context) {
	...
	channelOtherSettings, err := parseChannelOtherSettings(
		addChannelRequest.Channel.OtherSettings,
	)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf(
				"渠道额外设置[channel setting] 格式错误：%s",
				err.Error(),
			),
		})
		return
	...
	case "multi_to_single":
		...
		if isVertexServiceAccountChannel(
	...
	case "single":
		if isVertexServiceAccountChannel(
			...
			keys, err = parseVertexServiceAccountKeys(addChannelRequest.Channel.Key, false)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
...
func UpdateChannel(c *gin.Context) {
	...
	if _, ok := requestData["key_mode"]; ok &&
		(channel.KeyMode == nil ||
			(*channel.KeyMode != "append" && *channel.KeyMode != "replace")) {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": "key_mode must be either append or replace",
		})
		return
	}
	...
	requestOtherSettings, err := parseChannelOtherSettings(channel.OtherSettings)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf(
	...
	targetChannelType := originChannel.Type
	if _, provided := requestData["type"]; provided &&
		channel.Type != constant.ChannelTypeUnknown {
		targetChannelType = channel.Type
	}
	targetOtherSettings := dto.ChannelOtherSettings{}
	targetOtherSettingsKnown := targetChannelType != constant.ChannelTypeVertexAi
	_, requestOtherSettingsFieldPresent := requestData["settings"]
	requestOtherSettingsProvided := requestOtherSettingsFieldPresent &&
		channel.OtherSettings != ""
	if requestOtherSettingsProvided {
		targetOtherSettings = requestOtherSettings
		targetOtherSettingsKnown = true
	} else if targetChannelType == constant.ChannelTypeVertexAi &&
		originChannel.Type != constant.ChannelTypeVertexAi {
		targetOtherSettingsKnown = true
	}
	credentialDecisionRequested := keyFieldPresent ||
		targetChannelType != originChannel.Type ||
		requestOtherSettingsProvided
	originUsesVertexServiceAccount := false
	if originChannel.Type == constant.ChannelTypeVertexAi &&
		credentialDecisionRequested {
		originOtherSettings, parseErr := parseChannelOtherSettings(
			originChannel.OtherSettings,
		)
		if parseErr == nil {
			originUsesVertexServiceAccount = isVertexServiceAccountChannel(
				originChannel.Type,
				originOtherSettings,
			)
			if targetChannelType == constant.ChannelTypeVertexAi &&
				!targetOtherSettingsKnown {
				targetOtherSettings = originOtherSettings
				targetOtherSettingsKnown = true
			}
		} else if !targetOtherSettingsKnown {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "Stored channel settings are invalid",
			})
			return
		} else {
			// A malformed legacy Vertex mode is ambiguous. Treat it as a
			// service account for transition guards so credentials cannot be
			// reused as another provider's API key, while still allowing an
			// authorized request to replace the malformed settings.
			originUsesVertexServiceAccount = true
		}
	}
	targetsVertexServiceAccount := targetOtherSettingsKnown &&
		isVertexServiceAccountChannel(targetChannelType, targetOtherSettings)
	leavesVertexServiceAccount := originUsesVertexServiceAccount &&
		!targetsVertexServiceAccount
	if leavesVertexServiceAccount {
		if !keyFieldPresent || strings.TrimSpace(channel.Key) == "" {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "A replacement key is required when leaving Vertex AI service account mode",
			})
			return
		}
		if originChannel.ChannelInfo.IsMultiKey &&
			(channel.KeyMode == nil || *channel.KeyMode != "replace") {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "Vertex AI multi-key channels must use replace mode when leaving service account mode",
			})
			return
		}
	}
	if targetsVertexServiceAccount &&
		!keyFieldPresent &&
		!originUsesVertexServiceAccount {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Vertex AI service account key is required when enabling JSON key mode",
		})
		return
	}
	if keyFieldPresent &&
		...
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Vertex AI service account key cannot be blank",
		})
		return
	}
	keyProvided := keyFieldPresent && strings.TrimSpace(channel.Key) != ""
	vertexServiceAccountUpdate := keyProvided && targetsVertexServiceAccount

	if vertexServiceAccountUpdate {
		var normalizedKeys []string
		if channel.ChannelInfo.IsMultiKey &&
			channel.KeyMode != nil &&
			*channel.KeyMode == "append" {
			existingKeys, err := parseStoredVertexServiceAccountKeys(originChannel.Key)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
			...
			newKeys, err := parseVertexServiceAccountUpdateKeys(channel.Key, true)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
			...
			normalizedKeys = appendUniqueVertexServiceAccountKeys(existingKeys, newKeys)
			if err := validateVertexServiceAccountCollection(normalizedKeys); err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
				})
			...
			normalizedKeys, err = parseVertexServiceAccountUpdateKeys(
				channel.Key,
		...
		channel.Key = strings.Join(normalizedKeys, "\n")
	}
	...
	if _, settingProvided := requestData["setting"]; settingProvided {
		requestSettings, err := parseChannelSettings(channel.Setting)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "Channel proxy settings are invalid",
			})
			return
		}
		originSettings, originSettingsErr := parseChannelSettings(
			originChannel.Setting,
		)
		newProxy, _ := service.NormalizeProxyURL(requestSettings.Proxy)
		if originSettingsErr != nil {
			proxyChanged = true
		} else {
			originProxy = originSettings.Proxy
			normalizedOriginProxy, originProxyErr := service.NormalizeProxyURL(
				originProxy,
			)
			proxyChanged = originProxyErr != nil ||
				normalizedOriginProxy != newProxy
		}
	}
	if targetChannelType == constant.ChannelTypeVertexAi &&
		originChannel.Type != constant.ChannelTypeVertexAi &&
		!requestOtherSettingsProvided {
		channel.SetOtherSettings(targetOtherSettings)
	}
	err = channel.Update()
	if err != nil {
		common.ApiError(c, err)
...
func FetchModels(c *gin.Context) {
	...
	models, err := fetchChannelUpstreamModelIDsWithContext(
		c.Request.Context(),
		channel,
	)
	if err != nil {
...
const (
	multiKeyStatusDefaultPageSize = 50
	multiKeyStatusMaxPageSize     = 100
)
...
func ManageMultiKeys(c *gin.Context) {
	...
	if request.Action == "get_key_status" {
		...
		if request.PageSize <= 0 {
			request.PageSize = multiKeyStatusDefaultPageSize
		} else if request.PageSize > multiKeyStatusMaxPageSize {
			request.PageSize = multiKeyStatusMaxPageSize
		}
	}

	channel, err := model.GetChannelById(
		request.ChannelId,
