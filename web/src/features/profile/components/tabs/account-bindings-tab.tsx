...
export function AccountBindingsTab({
    ...
    try {
      ...
      const started = await startVerification(
        async (proofToken?: string) => {
          const response = await unbindCustomOAuth(
            target.provider_id,
            proofToken
          )
          if (!response.success) {
            throw new Error(response.message || t('Unbind failed'))
          }
          toast.success(
            t('Unbound {{provider}}', {
        ...
        {
          scope: 'external.unbind',
          target: target.provider_id,
          title: t('Security verification'),
        }
      )
      if (started) setUnbindTarget(null)
    } catch (error) {
  ...
  const startOAuthBinding = useCallback(
    async (provider: string, buildUrl: (state: string) => string) => {
      ...
      const started = await startVerification(
        async (proofToken?: string) => {
          try {
            if (pendingOAuthBinding.current !== pending || popup.closed) {
              throw new Error(t('Failed to initialize OAuth'))
            }
            const state = await createOAuthFlow(provider, 'bind', proofToken)
            if (pendingOAuthBinding.current !== pending || popup.closed) {
              return state
            }
            pending.state = state
            const authorizationUrl = buildUrl(state)
            if (!isSafeExternalHttpUrl(authorizationUrl)) {
              throw new Error(t('Failed to initialize OAuth'))
            }
    ...
    [clearPendingOAuthBinding, startVerification, t]
  )

  const handleVerification = useCallback(
    async (method: VerificationMethod, credential?: string) => {
      try {
  ...
  const handleVerificationCancel = useCallback(() => {
    ...
    cancelVerification()
  }, [cancelVerification, clearPendingOAuthBinding])
