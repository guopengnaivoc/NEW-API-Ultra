...
// updateUserCache refreshes user metadata in an existing cache entry.
...
// GetUserCache returns cached metadata with database-authoritative quota.
func GetUserCache(userId int) (*UserBase, error) {
	return getUserCache(userId, true)
}

func getUserMetadataCache(userId int) (*UserBase, error) {
	return getUserCache(userId, false)
}

func getUserCache(userId int, includeQuota bool) (*UserBase, error) {
	// Try getting from Redis first
	userCache, err := cacheGetUserBase(userId)
	if err == nil {
		if includeQuota {
			quota, quotaErr := getUserQuotaFromDB(userId)
			if quotaErr != nil {
				return nil, quotaErr
			}
...
func cacheGetUserBase(userId int) (*UserBase, error) {
	...
	userCache.Setting, err = openUserSettingValue(userCache.Setting)
	if err != nil {
		return nil, err
	}
	return &userCache, nil
...
func getUserGroupCache(userId int) (string, error) {
	cache, err := getUserMetadataCache(userId)
	if err != nil {
		return "", err
	}
...
func updateUserSettingCache(userId int, setting string) error {
	stored, cacheable, err := userSettingCacheValue(setting)
	if err != nil {
		return err
	}
	if !cacheable {
	...
	return updateUserCacheField(userId, "Setting", stored)
}
...
// GetUserLanguage returns the user's language preference from metadata cache.
func GetUserLanguage(userId int) string {
	userCache, err := getUserMetadataCache(userId)
	if err != nil {
		return ""
	}
