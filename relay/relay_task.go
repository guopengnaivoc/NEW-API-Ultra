...
type TaskSubmitResult struct {
	...
	TaskData          []byte
	ProviderResultURI string                           `json:"-"`
	SuccessResponse   *relaycommon.TaskSuccessResponse `json:"-"`
...
func ResolveOriginTask(c *gin.Context, info *relaycommon.RelayInfo) *dto.TaskError {
	...
	if info.OriginModelName == "" {
		info.OriginModelName = originTask.EffectiveModelName()
	}
...
// 控制器负责失败时 Refund，并在上游接受任务后原子链接任务与预扣操作。
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	if info.TaskRelayInfo == nil {
		info.TaskRelayInfo = &relaycommon.TaskRelayInfo{}
	}
	info.TaskRelayInfo.ProviderResultURI = ""
	info.TaskRelayInfo.SuccessResponse = nil
	...
	if resp != nil && resp.StatusCode != http.StatusOK {
		responseBody, readErr := service.ReadServiceResponseBody(resp, service.RelayErrorResponseMaxBytes)
		_ = resp.Body.Close()
		if info.ChannelType == constant.ChannelTypeGemini {
			message := fmt.Sprintf(
				"Gemini task submission failed: HTTP %d",
				resp.StatusCode,
			)
			if readErr == nil {
				...
				if sanitizeErr == nil {
					if sanitized.ErrorCode != 0 {
						message += fmt.Sprintf(", code %d", sanitized.ErrorCode)
					}
					if sanitized.ErrorStatus != "" {
						message += ", status " + sanitized.ErrorStatus
					}
				}
			}
	...
	return &TaskSubmitResult{
		...
		TaskData:          taskData,
		ProviderResultURI: info.TaskRelayInfo.ProviderResultURI,
		SuccessResponse:   info.TaskRelayInfo.SuccessResponse,
		Platform:          platform,
		Quota:             finalQuota,
	}, nil
}
...
func tryRealtimeFetch(
	ctx *gin.Context,
	task *model.Task,
	isOpenAIVideoAPI bool,
) []byte {
	...
	body, err := service.ReadServiceResponseBody(resp, service.VideoTaskPollingResponseMaxBytes)
	if err != nil {
		return nil
	}
	...
	if !snap.Equal(task.Snapshot()) {
		if task.Status == model.TaskStatusSuccess ||
			...
			if task.Status == model.TaskStatusSuccess {
				...
				requestedQuota, reason, clamp =
					service.CalculateTaskBillingOnComplete(
						ctx,
						adaptor,
						task,
						ti,
			...
			if _, err := service.ApplyDurableTaskBillingTransition(
				ctx,
				task,
				snap.Status,
				requestedQuota,
		...
		} else if _, err := task.UpdateWithStatus(snap.Status); err != nil {
			*task = originalTask
			return nil
		}
	}
...
func tryRealtimeFetchGemini(
	ctx *gin.Context,
	task *model.Task,
	channelModel *model.Channel,
	isOpenAIVideoAPI bool,
) []byte {
	resolvedKey, err := model.ResolveTaskChannelKey(channelModel, task)
	if err != nil {
		return nil
	}
	...
	resp, err := adaptor.FetchTask(baseURL, resolvedKey, map[string]any{
		"task_id": task.GetUpstreamTaskID(),
		"action":  task.Action,
	...
	updated.Data = append([]byte(nil), result.PublicData...)
	...
	if isDone {
		...
		*task = updated
	} else if !snapshot.Equal(updated.Snapshot()) {
	...
	if result.Failed || isOpenAIVideoAPI {
		return nil
	}

	format := result.VideoMIMEType
	if format == "" {
		format = "mp4"
	}
	compatibilityURL := ""
	if task.Status == model.TaskStatusSuccess {
		compatibilityURL = taskcommon.BuildProxyURL(task.TaskID)
	}
	out := map[string]any{
		"error":    nil,
		"format":   format,
...
func TaskModel2Dto(task *model.Task) *dto.TaskDto {
	...
	if !task.IsGeminiTask() {
		result.FailReason = task.FailReason
		result.ResultURL = task.GetResultURL()
	...
	if err != nil {
		terminal := task.Status == model.TaskStatusSuccess ||
			task.Status == model.TaskStatusFailure
		result.Data = geminitaskresult.EmptyPublicProjection(terminal)
	} else {
		result.Data = append([]byte(nil), sanitized.PublicData...)
	}
	...
	case model.TaskStatusFailure:
		result.FailReason = "upstream task failed"
	}
	return result
}
