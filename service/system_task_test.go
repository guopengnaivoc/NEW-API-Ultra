...
func TestPendingTwoFACleanupHandlerIsRegistered(t *testing.T) {
	var registered ScheduledSystemTaskHandler
	for _, handler := range registeredSystemTaskHandlers() {
		if handler.Type() != model.SystemTaskTypeTwoFACleanup {
			continue
		}
	...
	assert.Equal(t, time.Hour, registered.Interval())
}

func TestPendingTwoFACleanupHandlerRemovesExpiredCredentials(t *testing.T) {
	truncate(t)

	user := model.User{
		Username:    "scheduled-twofa-cleanup",
		Password:    "password",
		AffCode:     "scheduled-twofa-cleanup-aff",
		AuthVersion: 1,
	}
	require.NoError(t, model.DB.Create(&user).Error)
	factor := model.TwoFA{
		UserId:    user.Id,
		Secret:    "scheduled-expired-secret",
		IsEnabled: false,
	...
	require.NoError(t, model.DB.Create(&model.TwoFABackupCode{
		UserId:   user.Id,
		CodeHash: "scheduled-expired-code",
	}).Error)

	task, err := model.CreateSystemTask(
		model.SystemTaskTypeTwoFACleanup,
		nil,
		nil,
	)
	require.NoError(t, err)
	const runnerID = "twofa-cleanup-runner"
