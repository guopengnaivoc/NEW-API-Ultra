...
import { buildSecurityVerificationRequest } from './protocol'
import type {
  SecurityProof,
  SecurityProofScope,
  VerificationMethod,
...
export async function verify(
  ...
  switch (method) {
    case 'password':
      return verifyPassword(scope, target, credential)
    case '2fa':
      return verifyTwoFA(scope, target, credential)
    case 'passkey':
      return verifyPasskey(scope, target)
    default:
      throw new Error(
...
async function verifyTwoFA(
  ...
  const res = await api.post<ApiResponse<SecurityProof>>(
    '/api/verify',
    buildSecurityVerificationRequest('2fa', scope, target, trimmed)
  )
...
async function verifyPassword(
  ...
  const res = await api.post<ApiResponse<SecurityProof>>(
    '/api/verify',
    buildSecurityVerificationRequest('password', scope, target, password)
  )
  if (!res.data?.success) {
    throw new Error(res.data?.message || i18next.t('Verification failed'))
  }
...
async function verifyPasskey(
  ...
  try {
    const beginResponse = await beginPasskeyVerification(scope, target)
