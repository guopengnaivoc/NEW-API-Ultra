...
func OpenAIChatRequestToClaudeMessages(c context.Context, info convmeta.Meta, textRequest dto.GeneralOpenAIRequest) (*dto.ClaudeRequest, error) {
	...
	for _, tool := range textRequest.Tools {
		if params, ok := tool.Function.Parameters.(map[string]any); ok {
			...
			if params["type"] != nil {
				schemaType, ok := params["type"].(string)
				if !ok {
					return nil, converror.ClientInput(
						fmt.Errorf("tool %q parameters.type must be a string", tool.Function.Name),
					)
				}
	...
	if textRequest.Stop != nil {
		...
		case []interface{}:
			...
			for index, item := range stop {
				stopSequence, ok := item.(string)
				if !ok {
					return nil, converror.ClientInput(
						fmt.Errorf("stop[%d] must be a string", index),
					)
				}
