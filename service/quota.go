...
// PreWssConsumeQuota raises the durable reservation to cover cumulative
// realtime usage. Final websocket settlement remains the sole charge owner.
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, cumulativeUsage *dto.RealtimeUsage) error {
	if relayInfo == nil || cumulativeUsage == nil {
		return errors.New("invalid realtime billing state")
	}
	if relayInfo.UsePrice ||
		relayInfo.PriceData.UsePrice ||
		relayInfo.PriceData.FreeModel {
		return nil
	}

	textInputTokens := cumulativeUsage.InputTokenDetails.TextTokens
	textOutTokens := cumulativeUsage.OutputTokenDetails.TextTokens
	audioInputTokens := cumulativeUsage.InputTokenDetails.AudioTokens
	audioOutTokens := cumulativeUsage.OutputTokenDetails.AudioTokens
	...
	tieredOk, tieredQuota, _ := TryTieredSettle(relayInfo, billingexpr.TokenParams{
		P:   float64(cumulativeUsage.InputTokens),
		C:   float64(cumulativeUsage.OutputTokens),
		Len: float64(cumulativeUsage.InputTokens),
	})
	if tieredOk {
		quota = tieredQuota
	}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	audioRatio := decimal.NewFromFloat(priceData.AudioRatio)
	audioCompletionRatio := decimal.NewFromFloat(priceData.AudioCompletionRatio)
...
func CalcOpenRouterCacheCreateTokens(usage dto.Usage, priceData types.PriceData) int {
	...
	calculatedTokens := (cost -
		totalPromptTokens*quotaPrice +
		promptCacheReadTokens*(quotaPrice-promptCacheReadPrice) -
		completionTokens*completionPrice) /
		(promptCacheCreatePrice - quotaPrice)
	tokens, clamp := common.QuotaRoundChecked(calculatedTokens)
	if clamp != nil {
		return -1
	}
	return tokens
