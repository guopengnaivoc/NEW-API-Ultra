...
func StreamResponseClaude2OpenAIValidated(claudeResponse *dto.ClaudeResponse) (*dto.ChatCompletionsStreamResponse, error) {
	if claudeResponse != nil &&
		claudeResponse.Type == "content_block_delta" &&
		claudeResponse.Delta != nil &&
		claudeResponse.Delta.Type == "input_json_delta" &&
		claudeResponse.Delta.PartialJson == nil {
		return nil, converror.UpstreamResponse(
			errors.New("partial_json is required for input_json_delta"),
		)
	}
