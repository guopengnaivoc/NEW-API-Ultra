...
func TestSetUserPermissionsInTxIsAuthoritativeBeforeEnforcerReload(t *testing.T) {
	db := newAuthzTestDB(t)
	require.NoError(t, Init(db))
	...
	_, inMemory := explicitSubjectEffect(
		currentEnforcer(),
		UserSubject(42),
		ChannelSensitiveWrite,
	)
	assert.False(t, inMemory, "the transaction must not mutate the enforcer")
	assert.True(
		t,
		Can(42, common.RoleAdminUser, ChannelSensitiveWrite),
		"the committed database policy must be authoritative before reload",
	)
