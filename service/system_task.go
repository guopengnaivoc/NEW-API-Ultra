...
const (
	...
	pendingTwoFACleanupInterval  = time.Hour
	pendingTwoFACleanupBatchSize = 100
)
...
type pendingTwoFACleanupHandler struct{}

type pendingTwoFACleanupResult struct {
	DeletedCount int64 `json:"deleted_count"`
}

func (pendingTwoFACleanupHandler) Type() string {
	return model.SystemTaskTypeTwoFACleanup
}

func (pendingTwoFACleanupHandler) Enabled() bool {
	return true
}

func (pendingTwoFACleanupHandler) Interval() time.Duration {
	return pendingTwoFACleanupInterval
}

func (pendingTwoFACleanupHandler) NewPayload() any {
	return nil
}

func (pendingTwoFACleanupHandler) Run(
	ctx context.Context,
	task *model.SystemTask,
	runnerID string,
) {
	deleted, err := model.CleanupExpiredPendingTwoFASetups(
		ctx,
		time.Now(),
		pendingTwoFACleanupBatchSize,
	)
	if err != nil {
		failSystemTask(task, runnerID, err)
		return
	}
	result := pendingTwoFACleanupResult{DeletedCount: deleted}
	if err := model.FinishSystemTask(
		...
		logger.LogWarn(ctx, fmt.Sprintf(
			"system task %s failed to save twofa cleanup result: %v",
			task.TaskID,
			err,
		))
	}
...
func init() {
	...
	RegisterSystemTaskHandler(pendingTwoFACleanupHandler{})
}
