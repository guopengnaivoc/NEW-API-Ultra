...
func UnbindCustomOAuth(c *gin.Context) {
	...
	if err := model.DeleteUserOAuthBindingWithAuthVersion(userId, providerId); err != nil {
		common.ApiError(c, err)
		return
	}
	bundle, err := service.AdvanceCurrentSessionToUserVersion(identity, "external_identity_unbound")
	if err != nil {
		common.ApiError(c, err)
		return
	}
	recordUserSecurityAudit(c, userId, "user.external_identity_unbind", map[string]interface{}{
...
func UnbindCustomOAuthByAdmin(c *gin.Context) {
	...
	if !middleware.RequireOneTimeSecurityProof(
		...
		strconv.Itoa(userId)+":"+strconv.Itoa(providerId),
		[]string{
			secureVerificationMethodPassword,
			secureVerificationMethod2FA,
			secureVerificationMethodPasskey,
	...
	if _, err := model.RevokeAllUserSessions(userId, "admin_oauth_binding_clear"); err != nil {
		common.ApiError(c, err)
		return
	}
