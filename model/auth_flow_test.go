...
func TestCreateAuthFlowWithTxUsesCallerTransaction(t *testing.T) {
	truncateTables(t)
	rollbackErr := errors.New("roll back caller transaction")

	err := DB.Transaction(func(tx *gorm.DB) error {
		token, flow, createErr := CreateAuthFlowWithTx(tx, AuthFlowCreate{
