...
> **NEW API Ultra publication:** use [`DEPLOYMENT.md`](./DEPLOYMENT.md) for this repository's source build and Docker setup. The original New API/QuantumNous attribution and upstream links below are retained as required; do not substitute the upstream image when you intend to run this repository.

> Header badges that link to Calcium-Ion/QuantumNous are upstream attribution only; they do not identify this publication's release or container image.

> Upstream reference retained for attribution: the original source repository is [`QuantumNous/new-api`](https://github.com/QuantumNous/new-api) and its historical image is `calciumion/new-api:latest`. Those upstream clone/image instructions are not the deployment path for this publication.

## 📝 Project Description

> This repository is a stabilized publication snapshot. It is not a claim that every outstanding issue is resolved; use the exact tag and run the deployment checks before exposing a service publicly.
...
# Optional: review or edit .env before validation
# nano .env
...
Use [`DEPLOYMENT.md`](./DEPLOYMENT.md) for prebuilt-image or single-container instructions. The Compose path above is recommended because it supplies PostgreSQL, Redis, stable session/crypto secrets, and the data-encryption keyring. Do not copy a bare `docker run` command without those settings and persistent mounts.

</details>

---

🎉 After deployment is complete, visit `http://127.0.0.1:3000` to start using!
...
| `DATA_ENCRYPTION_KEYS` | Comma-separated `key-id=base64-key` root keyring for reversible secrets; every decoded key must be exactly 32 bytes and identical on all nodes | - |
| `DATA_ENCRYPTION_ACTIVE_KEY_ID` | Key identifier used for new envelopes and root-key rewrap | - |
| `DATA_ENCRYPTION_ENABLE` | At-rest encryption enforcement and migration gate; disable only for the documented mixed-version preparation phase | `true` |
...
2. Follow this repository's [`DEPLOYMENT.md`](./DEPLOYMENT.md) or the
   [`BaoTa tutorial`](./docs/installation/BT.md): clone `NEW-API-Ultra`, generate
   `.env`, and start the repository's Docker Compose services.
3. The BaoTa application-store **New-API** one-click package and upstream
   images are reference paths only; they do **not** deploy this snapshot.
...
See [Reversible secret encryption and key rotation](./docs/data-encryption.md)
before upgrading an existing deployment or rotating its data-encryption key.

For a Redis-backed multi-node upgrade across the relay-token cache namespace
change, follow the [token cache rolling-upgrade procedure](./docs/token-cache-rolling-upgrade.md).
...
**[Deployment Guide](./DEPLOYMENT.md)** • **[Publication Issues](https://github.com/guopengnaivoc/NEW-API-Ultra/issues)** • **[Publication Releases](https://github.com/guopengnaivoc/NEW-API-Ultra/releases)** • **[Upstream Documentation](https://docs.newapi.pro/en/docs)** • **[Upstream Issues](https://github.com/Calcium-Ion/new-api/issues)** • **[Upstream Releases](https://github.com/Calcium-Ion/new-api/releases)**
