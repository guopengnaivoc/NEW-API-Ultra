...
func TestGetClaudeSettingsSuppliesDefaultWithoutMutatingPublishedSnapshot(t *testing.T) {
	original, err := config.ConfigToMap(config.Snapshot[ClaudeSettings]("claude"))
	require.NoError(t, err)
	t.Cleanup(func() {
		updated, updateErr := config.GlobalConfig.Update("claude", original)
		require.NoError(t, updateErr)
		require.True(t, updated)
	})
	updated, err := config.GlobalConfig.Update("claude", map[string]string{
		"default_max_tokens": `{"claude-model":4096}`,
	})
	require.NoError(t, err)
	require.True(t, updated)
	published := config.Snapshot[ClaudeSettings]("claude")
	_, hasPublishedDefault := published.DefaultMaxTokens["default"]
	require.False(t, hasPublishedDefault)

	derived := GetClaudeSettings()

	assert.NotSame(t, published, derived)
	assert.Equal(t, 4096, derived.DefaultMaxTokens["claude-model"])
	assert.Equal(t, 8192, derived.DefaultMaxTokens["default"])
	_, hasPublishedDefault = published.DefaultMaxTokens["default"]
	assert.False(t, hasPublishedDefault)
}
