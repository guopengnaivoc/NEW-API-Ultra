...
func GetFileTypeFromUrl(c *gin.Context, url string, reason ...string) (string, error) {
	...
	if err != nil {
		common.SysLog(fmt.Sprintf("fail to get file type from %s", source.GetIdentifier()))
		return "", err
	}
	defer response.Body.Close()

	if response.StatusCode != 200 {
		logger.LogError(c, fmt.Sprintf("failed to download file from %s, status code: %d", source.GetIdentifier(), response.StatusCode))
		return "", types.NewFileSourceError(types.FileSourceErrorCategoryUnexpectedStatus, source, response.StatusCode)
	}
...
func GetFileBase64FromUrl(c *gin.Context, url string, reason ...string) (*types.LocalFileData, error) {
	...
	if err != nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryCacheReadFailed, source)
	}
