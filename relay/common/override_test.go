...
func TestApplyParamOverrideConditionFromSafeRequestHeaders(t *testing.T) {
	input := []byte(`{"temperature":0.7}`)
	override := map[string]interface{}{
		"operations": []interface{}{
			map[string]interface{}{
				...
				"conditions": []interface{}{
					map[string]interface{}{
						"path":  "request_headers.x-request-id",
						"mode":  "contains",
						"value": "request-",
					},
				},
...
func TestApplyParamOverrideConditionsCannotInspectSensitiveRequestHeaders(t *testing.T) {
	t.Parallel()
	...
	ctx := map[string]interface{}{
		"request_headers": map[string]interface{}{
			...
			"x-auth-session": "caller-session-id",
		},
	}
	...
	requestHeaders, ok := ctx[paramOverrideContextRequestHeaders].(map[string]interface{})
	require.True(t, ok)
	require.NotContains(t, requestHeaders, "authorization")
	require.NotContains(t, requestHeaders, "x-auth-session")
}
	...
	override := map[string]interface{}{
		"operations": []interface{}{
			...
			map[string]interface{}{
				...
				"conditions": []interface{}{
					map[string]interface{}{
						"path":  "header_override.x-upstream-request-id",
						"mode":  "contains",
						"value": "request-",
					},
				},
...
func TestApplyParamOverrideWithRelayInfoSuppressesSensitiveClientHeaderSources(t *testing.T) {
	t.Parallel()
	...
	info := &RelayInfo{
		RequestHeaders: map[string]string{
			...
			"X_API_KEY":              "caller-api-key",
			"X-Security-Proof":       "caller-security-proof",
			"Sec_WebSocket_Protocol": "openai-insecure-api-key.caller-ws-key",
			...
			ParamOverride: map[string]interface{}{
				"operations": []interface{}{
					...
					map[string]interface{}{
						...
						"to":   "json:stolen_token",
					},
				},
			},
			HeadersOverride: map[string]interface{}{
				"X-Static": "channel-owned",
			},
		},
	}
	...
	require.Equal(t, "channel-owned", info.RuntimeHeadersOverride["x-static"])
	...
	for _, forbidden := range []string{
		...
		"x-stolen-authorization",
		"x-stolen-cookie",
	} {
		require.NotContains(t, info.RuntimeHeadersOverride, forbidden)
	}
}

func TestBuildParamOverrideContextFiltersSensitiveClientHeaders(t *testing.T) {
	t.Parallel()

	info := &RelayInfo{
		RequestHeaders: map[string]string{
			"Authorization":       "Bearer caller-token",
			"proxy_authorization": "Basic caller-proxy-token",
			"X-Goog-Api-Key":      "caller-api-key",
			"X-Auth-Session":      "caller-session-id",
			"X-Turnstile-Token":   "caller-turnstile-token",
	...
	requestHeaders, ok := ctx[paramOverrideContextRequestHeaders].(map[string]interface{})
	require.True(t, ok)
	require.Equal(t, "trace-123", requestHeaders["x-trace-id"])
	for _, forbidden := range []string{
		"authorization",
		"proxy_authorization",
		"proxy-authorization",
		...
		require.NotContains(t, requestHeaders, forbidden)
	}
}

func TestGetHeaderValueFromContextAllowsChannelOwnedCredentialOnly(t *testing.T) {
	t.Parallel()
	...
	value, ok := getHeaderValueFromContext(ctx, "Authorization")
	require.True(t, ok)
	require.Equal(t, "Bearer provider-token", value)

	for _, protectedName := range []string{"X-Api-Key", "X_Auth_Session"} {
		value, ok = getHeaderValueFromContext(ctx, protectedName)
		require.False(t, ok)
		require.Empty(t, value)
	}

	value, ok = getHeaderValueFromContext(ctx, "X-Request-Id")
	require.True(t, ok)
...
func TestRemoveDisabledFieldsSkipWhenGlobalPassThroughEnabled(t *testing.T) {
	original, err := config.ConfigToMap(model_setting.GetGlobalSettings())
	require.NoError(t, err)
	t.Cleanup(func() {
		updated, updateErr := config.GlobalConfig.Update("global", original)
		require.NoError(t, updateErr)
		require.True(t, updated)
	})
	updated, err := config.GlobalConfig.Update("global", map[string]string{
