...
import { startTwoFAChallenge } from '@/features/auth/lib/login-result'
...
export function useAuthRedirect() {
  ...
  const beginTwoFAChallenge = useCallback(
    (data: unknown): boolean => {
      const result = startTwoFAChallenge(data, {
        setPendingFlowToken: setPending2FAFlowToken,
        navigateToTwoFA: () => navigate({ to: '/otp', replace: true }),
      })
      if (result === 'invalid') {
    ...
    [navigate, setPending2FAFlowToken, t]
  )
