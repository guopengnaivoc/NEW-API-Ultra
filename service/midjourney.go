...
func (err *MidjourneyRequestError) Error() string {
	return err.err.Error()
}

func (err *MidjourneyRequestError) Unwrap() error {
	return err.err
}

func MidjourneyRequestWasDispatched(err error) bool {
	var requestErr *MidjourneyRequestError
...
func DoMidjourneyHttpRequest(c *gin.Context, timeout time.Duration, fullRequestURL string) (*dto.MidjourneyResponseWithStatusCode, []byte, error) {
	...
	if c.Request.Method != "GET" {
		...
		if err != nil {
			return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "read_request_body_failed", http.StatusInternalServerError), nullBytes, &MidjourneyRequestError{
				err:        err,
				dispatched: false,
			}
		}
	...
	if err != nil {
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "marshal_request_body_failed", http.StatusInternalServerError), nullBytes, &MidjourneyRequestError{
			err:        err,
			dispatched: false,
		}
	}
	...
	if err != nil {
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "create_request_failed", http.StatusInternalServerError), nullBytes, &MidjourneyRequestError{
			err:        err,
			dispatched: false,
		}
	}
	...
	if err != nil {
		...
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "do_request_failed", http.StatusInternalServerError), nullBytes, &MidjourneyRequestError{
			err:        err,
			dispatched: true,
		}
	}
	...
	err = req.Body.Close()
	if err != nil {
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "close_request_body_failed", statusCode), nullBytes, &MidjourneyRequestError{
			err:        err,
			dispatched: true,
		}
	}
	...
	responseBody, err := ReadMidjourneyResponseBody(resp)
	if err != nil {
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "read_response_body_failed", statusCode), nullBytes, &MidjourneyRequestError{
			err:        err,
			dispatched: true,
		}
	}
	...
	if len(responseBody) == 0 {
		return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "empty_response_body", statusCode), responseBody, &MidjourneyRequestError{
			err:        errors.New("empty Midjourney response body"),
			dispatched: true,
		}
	} else {
		...
		if err != nil {
			...
			if err2 != nil {
				return MidjourneyErrorWithStatusCodeWrapper(constant.MjErrorUnknown, "unmarshal_response_body_failed", statusCode), responseBody, &MidjourneyRequestError{
					err:        err,
					dispatched: true,
				}
			}
