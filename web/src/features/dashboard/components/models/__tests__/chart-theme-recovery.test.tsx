...
test('a dashboard chart recovers in place after a transient theme failure', async () => {
  ...
  try {
    await act(async () => {
      root.render(
        <I18nextProvider i18n={i18n}>
          <ThemeProvider
            defaultTheme='dark'
            storageKey='dashboard-chart-theme-test'
          >
            <ModelCharts data={[]} loading />
          </ThemeProvider>
        </I18nextProvider>
      )
    ...
    assert.ok(retry)
    assert.equal(container.querySelector('[data-chart-rendered]'), null)
    ...
    assert.equal(container.querySelector('[role="alert"]'), null)
    assert.ok(container.querySelector('[data-chart-rendered]'))
  } finally {
