...
type sessionCacheCommitObservationHook struct {
	db             *gorm.DB
	observedCounts []int64
	err            error
}

func (hook *sessionCacheCommitObservationHook) observeCommand(command redis.Cmder) {
	for _, argument := range command.Args() {
		key, ok := argument.(string)
		if !ok || !strings.HasPrefix(key, "auth:session:") {
			continue
		}
		var count int64
		if err := hook.db.Model(&model.UserSession{}).Count(&count).Error; err != nil {
			hook.err = err
			return
		}
		hook.observedCounts = append(hook.observedCounts, count)
		return
	}
}

func (hook *sessionCacheCommitObservationHook) BeforeProcess(ctx context.Context, command redis.Cmder) (context.Context, error) {
	hook.observeCommand(command)
	return ctx, nil
}

func (*sessionCacheCommitObservationHook) AfterProcess(context.Context, redis.Cmder) error {
	return nil
}

func (hook *sessionCacheCommitObservationHook) BeforeProcessPipeline(ctx context.Context, commands []redis.Cmder) (context.Context, error) {
...
func (*sessionCacheCommitObservationHook) AfterProcessPipeline(context.Context, []redis.Cmder) error {
	return nil
}
...
func setupConcurrentAuthSessionTestDB(t *testing.T) *model.User {
	t.Helper()
	...
	databasePath := filepath.Join(t.TempDir(), "auth-session-admission.db")
...
func gateConcurrentUserSessionAdmissions(t *testing.T, expectedCallers int32) {
	t.Helper()

	updateCallbackName := "test:gate_concurrent_user_session_reservations"
	createCallbackName := "test:gate_concurrent_user_session_creates"
	...
	gate := func(tx *gorm.DB) {
		...
		isAdmissionReservation := tx.Statement.Table == "users"
		isSessionCreate := tx.Statement.Table == "user_sessions"
		if !isAdmissionReservation && !isSessionCreate {
			return
		}
		select {
		case <-release:
		...
		if arrivals.Add(1) == expectedCallers {
			closeRelease()
		}
		<-release
	}
	require.NoError(t, model.DB.Callback().Update().Before("gorm:update").Register(updateCallbackName, gate))
	require.NoError(t, model.DB.Callback().Create().Before("gorm:create").Register(createCallbackName, gate))
	t.Cleanup(func() {
		closeRelease()
		_ = model.DB.Callback().Update().Remove(updateCallbackName)
		_ = model.DB.Callback().Create().Remove(createCallbackName)
	})
}
...
func TestConcurrentLoginSessionAdmissionRespectsLimits(t *testing.T) {
	testCases := []struct {
		name          string
		activeLimit   int
		issuanceLimit int
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			useTestSessionSecret(t)
			user := setupConcurrentAuthSessionTestDB(t)
			common.UserSessionActiveLimit = testCase.activeLimit
			common.UserSessionIssuanceLimit = testCase.issuanceLimit
			common.UserSessionIssuanceWindowSeconds = 60
			gateConcurrentUserSessionAdmissions(t, 2)
			...
			for index := range 2 {
				go func(index int) {
					<-start
					_, err := CreateLoginSession(
						user.Id,
						"password",
						"127.0.0.1",
						fmt.Sprintf("concurrent-agent-%d", index),
					)
					results <- err
				}(index)
			}
			...
			for range 2 {
				...
				case <-time.After(5 * time.Second):
					require.FailNow(t, "concurrent login did not finish")
				}
			}

			assert.Equal(t, 1, successes, "only one concurrent admission may commit")
			var count int64
			require.NoError(t, model.DB.Model(&model.UserSession{}).Count(&count).Error)
...
func TestCreateLoginSessionUsesAuthoritativeUserState(t *testing.T) {
	testCases := []struct {
		name          string
		mutateUser    func(t *testing.T, user *model.User)
		createSession func(user *model.User) error
		wantErr       error
	}{
		{
			name: "disabled user",
			...
			createSession: func(user *model.User) error {
				_, err := CreateLoginSession(user.Id, "password", "127.0.0.1", "stale-cache-agent")
				return err
			},
			wantErr: ErrLoginSessionInvalid,
		},
		{
			name: "advanced auth version",
			mutateUser: func(t *testing.T, user *model.User) {
				require.NoError(t, model.DB.Model(&model.User{}).
					Where("id = ?", user.Id).
			...
			createSession: func(user *model.User) error {
				_, err := CreateLoginSessionAtAuthVersion(
					user.Id,
					user.AuthVersion,
					"2fa",
					"127.0.0.1",
			...
			wantErr: ErrLoginSessionRevoked,
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			testCase.mutateUser(t, user)
			err = testCase.createSession(user)
...
func TestCreateLoginSessionAcquiresSQLiteReservationBeforeCounting(t *testing.T) {
	useTestSessionSecret(t)
	user := setupConcurrentAuthSessionTestDB(t)

	reservationObserved := false
	updateCallbackName := "test:observe_user_session_admission_reservation"
	require.NoError(t, model.DB.Callback().Update().Before("gorm:update").Register(updateCallbackName, func(tx *gorm.DB) {
		if tx.Statement != nil && tx.Statement.Table == "users" {
			reservationObserved = true
		}
	}))
	queryCallbackName := "test:require_user_session_admission_reservation"
	require.NoError(t, model.DB.Callback().Query().Before("gorm:query").Register(queryCallbackName, func(tx *gorm.DB) {
		if tx.Statement != nil && tx.Statement.Table == "user_sessions" && !reservationObserved {
			tx.AddError(errors.New("session count ran before SQLite write reservation"))
		}
	}))
	t.Cleanup(func() {
		_ = model.DB.Callback().Update().Remove(updateCallbackName)
		_ = model.DB.Callback().Query().Remove(queryCallbackName)
	})

	_, err := CreateLoginSession(user.Id, "password", "127.0.0.1", "reservation-agent")
	require.NoError(t, err)
	assert.True(t, reservationObserved)
}

func TestCreateLoginSessionRequestsExternalDatabaseRowLock(t *testing.T) {
	useTestSessionSecret(t)
	user := setupAuthSessionTestDB(t)
	previousMainDatabaseType := common.MainDatabaseType()
	previousLogDatabaseType := common.LogDatabaseType()
	...
	callbackName := "test:observe_user_session_admission_row_lock"
	require.NoError(t, model.DB.Callback().Query().Before("gorm:query").Register(callbackName, func(tx *gorm.DB) {
		if tx.Statement == nil || tx.Statement.Table != "users" {
			return
		}
		_, rowLockObserved = tx.Statement.Clauses["FOR"]
	}))
	t.Cleanup(func() {
		_ = model.DB.Callback().Query().Remove(callbackName)
	})

	_, err := CreateLoginSession(user.Id, "password", "127.0.0.1", "row-lock-agent")
	require.NoError(t, err)
	assert.True(t, rowLockObserved)
}

func TestCreateLoginSessionRollbackDoesNotPublishCache(t *testing.T) {
	useTestSessionSecret(t)
	user := setupAuthSessionTestDB(t)
	...
	callbackName := "test:fail_user_session_insert_after_create"
	...
	_, err := CreateLoginSession(user.Id, "password", "127.0.0.1", "rollback-agent")
	assert.ErrorIs(t, err, forcedErr)

	var count int64
	require.NoError(t, model.DB.Model(&model.UserSession{}).Count(&count).Error)
	assert.Zero(t, count)
	for _, key := range serverA.Keys() {
		assert.NotContains(t, key, "auth:session:")
	}
}

func TestCreateLoginSessionPublishesCacheAfterCommit(t *testing.T) {
	useTestSessionSecret(t)
	user := setupConcurrentAuthSessionTestDB(t)
	_, clientA, _, _ := useIndependentAuthSessionRedis(t)
	common.RDB = clientA
	observation := &sessionCacheCommitObservationHook{db: model.DB}
	clientA.AddHook(observation)

	_, err := CreateLoginSession(user.Id, "password", "127.0.0.1", "commit-order-agent")
	require.NoError(t, err)
	require.NoError(t, observation.err)
	require.NotEmpty(t, observation.observedCounts)
	for _, count := range observation.observedCounts {
		assert.Equal(t, int64(1), count, "cache publication must observe the committed session row")
	}
}
