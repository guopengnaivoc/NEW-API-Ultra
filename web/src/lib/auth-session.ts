...
import { clearPlaygroundData } from '@/features/playground/lib/storage/storage'
...
const authRefreshDeadlineMs = 10_000
...
export function applyAuthBundle(
  ...
  const previousSID = previousAuth.session?.sid
  const previousUserId = previousAuth.user?.id
  authEpoch += 1
  // A different account signing in without an explicit sign-out must not
  // inherit the previous user's persisted playground conversations
  // (NA-ISSUE-0080).
  if (previousUserId !== undefined && previousUserId !== bundle.user.id) {
    clearPlaygroundData()
  }
...
export function clearAuthentication(
  ...
  // Playground conversations persist in plaintext localStorage; remove them
  // whenever the authenticated session ends so the next user on this browser
  // cannot read the previous user's chats (NA-ISSUE-0080).
  clearPlaygroundData()
...
export function waitForRefreshRace(
  delay: number,
  signal: AbortSignal
): Promise<void> {
...
async function performRefreshWithBrowserLock(
  ...
  const deadline = globalThis.setTimeout(
    ...
    authRefreshDeadlineMs
  )
  try {
    if (typeof navigator === 'undefined' || !navigator.locks) {
      return await runRefresh(refreshEpoch, abortController.signal)
    }
    return await navigator.locks.request(
      'new-api:auth-refresh',
      { mode: 'exclusive', signal: abortController.signal },
      () => runRefresh(refreshEpoch, abortController.signal)
    )
  } catch (error: unknown) {
    useAuthStore.getState().auth.setBootstrapState('idle')
    return { kind: 'transient_error', error }
