...
- Refresh Token 是随机不透明值，绝对有效期最长 30 天。登录 Session 默认连续 7 天没有成功刷新即因闲置失效；浏览器只通过 `HttpOnly`、`SameSite=Strict` Cookie 持有它，服务端仅保存 HMAC 摘要，并在每次成功刷新时轮换。
- `user_sessions` 是登录会话控制面，记录设备、IP、登录方式、最后活跃时间、绝对到期时间和撤销状态。有效截止时间取 `expires_at` 与 `last_active_at + USER_SESSION_IDLE_TIMEOUT_SECONDS` 中较早者，到达截止时间的当秒即失效。数据库中的 Session 状态是最终权威；撤销传播速度取决于下文所述的 Redis 拓扑。
...
`SESSION_SECRET` 用于派生 Access Token、Security Proof、Refresh Token 摘要和 AuthFlow 摘要的不同用途密钥。生产环境及多节点部署必须在所有节点配置相同的高强度随机值；更换该值会使现有登录、临时鉴权流程和 Security Proof 全部失效。成功刷新是普通登录 Session 唯一的滑动活跃事件；使用 Access Token 的普通面板请求会校验闲置截止时间，但不会单独更新 `last_active_at`。

## 多节点 Redis 拓扑

多节点部署必须共用同一主数据库。登录 Session、账户级活跃 Session 上限和签发窗口计数都以数据库为权威，因此这些限制在应用节点间全局生效。Redis 中的 Session Hash（包含 `revoking`/`revoked` tombstone）只是缓存，其 TTL 为 Session 有效剩余寿命（同时受绝对与闲置截止时间约束）与有效 `SYNC_FREQUENCY` 中的较小值；`SYNC_FREQUENCY` 默认及非法值回退均为 `60` 秒。读取缓存不会续期，过期后会按 SID 回源数据库。延迟完成的 active 缓存回写只能使用其数据库观察窗口尚未消耗的 TTL，不能在撤销 tombstone 到期后重新启动一个完整缓存周期。
...
刷新请求到达闲置截止时间时，无论携带当前 Refresh Token 还是短暂容错窗口内的上一代 Token，都不会轮换令牌或更新最后活跃时间。服务端会以 `idle_timeout` 原因持久撤销仍处于闲置状态的权威 Session，并返回 `401 AUTH_SESSION_REVOKED`、清除 Cookie。Redis 中较旧的闲置快照会先回源数据库复核；如果数据库记录已经被并发刷新推进，不会错误撤销新状态。
...
- `USER_SESSION_IDLE_TIMEOUT_SECONDS`（默认 `604800`，即 7 天）：必须为正数；未配置、非法、零或负数均回退默认值。它只滑动闲置截止时间，不能延长固定的 30 天绝对到期时间。大于 30 天的配置仍受绝对到期时间限制。
- 权威用户状态、两次计数与插入在同一事务内按用户串行化；SQLite 先取得写预留，MySQL/PostgreSQL 使用用户行 `FOR UPDATE`。锁、计数或插入失败都会拒绝签发，不会降级放行。

升级时已经超过活跃上限的账户不会被自动下线或挤掉旧会话；限制只作用于后续的新 Session 签发。活跃数量和设备列表会立即排除到达闲置截止时间的行，即使定时清理尚未处理它们；签发窗口仍统计这些已签发行。

升级会立即使用现有行的 `last_active_at` 计算闲置截止时间。需要渐进启用时，可先在所有节点统一配置一个较长的正数，完成全量升级后再统一降低。所有节点必须使用相同的 `USER_SESSION_IDLE_TIMEOUT_SECONDS`；滚动升级期间，旧版本进程在新版本进程尚未遇到并持久撤销某个闲置 Session 前仍可能按旧规则接受它，因此严格边界要求协调发布或排空旧节点。回滚旧二进制不会恢复已经以 `idle_timeout` 撤销的 Session，用户需重新登录；尚未被持久撤销的 active 行则会恢复旧版本的无闲置过期行为。
...
邮箱验证和密码重置使用 256 位随机 AuthFlow 凭据。数据库只持久化凭据的 HMAC 摘要，以及与用途绑定的目标哈希载荷，不保存明文凭据、邮箱或密码。凭据会过期且只能使用一次；受保护的用户变更与流程消费在同一数据库事务中提交。

- `POST /api/verification` 和 `POST /api/reset_password` 从 JSON 请求体接收 `{ "email": "..." }`；启用 Turnstile 时可通过可选的 `X-Turnstile-Token` 请求头提交令牌。
- `POST /api/user/reset` 从 JSON 请求体接收 `{ "token": "...", "password": "..." }`；密码必须为 8–20 个 Unicode 字符，且 UTF-8 编码不得超过 72 字节。成功响应不包含密码、生成密码或其他密钥数据。
- 重置邮件只包含规范的 `/user/reset` 链接，不在查询参数或片段中携带凭据；重置码会在邮件中单独提供。

成功重置密码会推进 `auth_version`，使现有登录会话和面板 PAT 在尽力清理会话行之前立即失效。协调发布会有意使旧的进程内邮箱验证和密码重置凭据失效；这些旧凭据对外声明的最长有效期为十分钟，用户随后可重新申请。所有共享数据库的节点必须使用相同配置的 `SESSION_SECRET`。
...
敏感操作使用有效期 5 分钟的 `X-Security-Proof`。每一种受支持的
Security Proof 都是一个不透明、数据库支持、精确目标、一次性消费的
`AuthFlow`，而不是可重用的 JWT。服务端将其同时绑定到用户、仍有效的
浏览器登录会话、用户鉴权版本、会话版本、验证方式、scope 和 target；它不能
跨用户、跨会话、跨验证方式、跨用途或跨目标复用。

以下只列出三个代表性 scope，并非服务端支持范围的完整清单：

- `channel.key.read`：查看渠道密钥。target 必须是渠道路径 ID 的正规范十进制
  表示（例如 `17`，不接受 `017`、符号、空白或零），并且只能通过 2FA 或
  Passkey 签发；
- `passkey.register`：注册 Passkey；
- `passkey.delete`：删除 Passkey。

渠道密钥接口会在控制器执行前验证并消费与该精确路径 ID 匹配的 Proof。scope、
验证方式或 target 不匹配时不会消费 Proof，因此用户可用同一 Proof 重试正确的
目标；一次成功验证后，无论后续控制器处理或响应交付是否失败，该 Proof 都不能
再次使用。
...
- Security Proof 从旧的五分钟 JWT 迁移为目标绑定的一次性 AuthFlow 时，必须先
  排空或替换所有旧节点，并等待旧版五分钟 JWT TTL 完全过去，才能宣布该安全
  边界在滚动升级中完全关闭。
- 新节点仍保留对旧 `token_use=security_proof` JWT 的内部凭据分类；此类旧
  Proof 会直接按无效凭据失败关闭，不会回退为面板 PAT 或其他鉴权方式。
