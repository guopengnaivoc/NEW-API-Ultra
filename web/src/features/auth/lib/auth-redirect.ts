...
export function buildSessionRecoverySignInHref(
  ...
  const safeRedirect = sanitizeAuthRedirect(currentHref, origin)
  ...
  if (normalizedPathname === '/sign-in') return null
  if (
    authFlowPaths.has(normalizedPathname) ||
    normalizedPathname === '/oauth' ||
    normalizedPathname.startsWith('/oauth/')
  ) {
    return '/sign-in'
  }
