...
const (
	...
	QuotaClampCapacity  QuotaClampKind = "capacity"
)

// QuotaClamp describes a quota saturation event caused by conversion bounds,
// NaN fallback, or available billing capacity. It is surfaced to billing
// callers so the event can be recorded on the related consume/task log for
// admin auditing.
type QuotaClamp struct {
	Op       string         `json:"op"`       // conversion helper or billing operation name
	Kind     QuotaClampKind `json:"kind"`     // "overflow" | "underflow" | "nan" | "capacity"
...
// QuotaFromDecimalStrict converts an in-range decimal quota and returns a
// typed *QuotaClamp error instead of allowing a saturated value to reach a
// financial mutation.
