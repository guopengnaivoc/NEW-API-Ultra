...
import { ChartThemeError } from '@/components/chart-theme-error'
...
export function ModelsSection(props: ModelsSectionProps) {
  ...
  const { resolvedTheme, retryTheme, themeError, themeReady } = useChartTheme()
  ...
  if (themeError && spec) {
    chartContent = <ChartThemeError onRetry={retryTheme} />
  }
  ...
  return (
    <section className='bg-card overflow-hidden rounded-lg border'>
      ...
      <div className='px-5 pb-5'>
        <div className='h-60 sm:h-72'>{chartContent}</div>
      </div>
