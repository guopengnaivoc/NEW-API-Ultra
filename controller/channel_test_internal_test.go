...
func TestSettleTestQuotaRecordsRatioClamp(t *testing.T) {
	info := &relaycommon.RelayInfo{}
	quota, result := settleTestQuota(info, types.PriceData{
		ModelRatio:      math.MaxFloat64,
		CompletionRatio: 1,
	}, &dto.Usage{PromptTokens: 2})
...
func TestSettleTestQuotaRecordsFixedPriceClamp(t *testing.T) {
	info := &relaycommon.RelayInfo{}
	quota, result := settleTestQuota(info, types.PriceData{
		UsePrice:   true,
...
func TestBuildTestLogOtherAttachesQuotaSaturation(t *testing.T) {
	ctx, _ := gin.CreateTestContext(httptest.NewRecorder())
	info := &relaycommon.RelayInfo{
		ChannelMeta: &relaycommon.ChannelMeta{},
