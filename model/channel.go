...
type Channel struct {
	...
	EncryptedKey       string  `json:"-" gorm:"column:key;not null"`
...
const channelKeySecretDomain = "channels:key"

// ErrChannelKeyRotationConflict reports that the encrypted credential changed
// after the rotation operation acquired its authoritative snapshot.
var ErrChannelKeyRotationConflict = errors.New("channel key rotation conflict")

// ChannelKeyRotationFunc computes a replacement from a locked authoritative
// channel. Returning changed=false commits without rewriting the credential.
type ChannelKeyRotationFunc func(current *Channel) (nextKey string, changed bool, err error)

func (channel *Channel) BeforeSave(_ *gorm.DB) error {
	stored, err := common.SealDataEncryptionValue(
...
func (channel *Channel) GetNextEnabledKey() (string, int, *types.NewAPIError) {
	...
	case constant.MultiKeyModePolling:
		// The cursor lives in channelPollingCursors rather than on the channel
		// struct. With the memory cache on, this *Channel is the very pointer
		// published to every other reader, so advancing the cursor in place
		// would be a write racing every lock-free field read of the selected
		// channel. The rotation is process-local either way: it was never
		// persisted per request, and restarting simply restarts the rotation.
		var start int
		...
		for i := 0; i < len(keys); i++ {
			...
			if getStatus(idx) == common.ChannelStatusEnabled {
				...
				if common.DebugEnabled {
					logger.LogDebug(nil, "channel %d polling index: %d", channel.Id, next)
				}
...
func UpdateChannelKey(channelID int, key string) error {
	stored, err := common.SealDataEncryptionValue(channelKeySecretDomain, key)
	if err != nil {
		return err
	}
	return DB.Model(&Channel{}).
		Where("id = ?", channelID).
		Update("key", stored).Error
}

// RotateChannelKey serializes a provider credential rotation through the
// primary database and persists the encrypted replacement with compare-and-swap.
func RotateChannelKey(
	ctx context.Context,
	channelID int,
	rotate ChannelKeyRotationFunc,
) (*Channel, bool, error) {
	if ctx == nil {
		return nil, false, errors.New("channel key rotation context is required")
	}
	if channelID <= 0 {
		return nil, false, errors.New("channel key rotation requires a valid channel id")
	}
	if rotate == nil {
		return nil, false, errors.New("channel key rotation callback is required")
	}
	...
	err := DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if common.UsingMainDatabase(common.DatabaseTypeSQLite) {
			...
			if result.RowsAffected != 1 {
				return fmt.Errorf(
					"channel key rotation reserved %d rows",
					result.RowsAffected,
				)
			}
		}
		...
		originalEnvelope := current.EncryptedKey
		callbackChannel := current
		nextKey, shouldChange, err := rotate(&callbackChannel)
		if err != nil {
			return err
		}
		if !shouldChange {
		...
		predicate := map[string]any{"id": channelID}
		if common.DataEncryptionEnabled() {
			predicate["key"] = originalEnvelope
		}
		result := tx.Model(&Channel{}).
			Where(predicate).
			UpdateColumn("key", stored)
		if result.Error != nil {
			return result.Error
		}
		if result.RowsAffected != 1 {
...
// channelPollingCursors keeps the multi-key rotation index per channel. It is
// process-local by design: the cursor must not be written onto the cached
// *Channel, which is shared with every concurrent reader, and persisting it per
// relay request would cost one database write per call.
...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	if common.MemoryCacheEnabled {
		...
		if channelCache.ChannelInfo.IsMultiKey {
			// The pointer CacheGetChannel returns is shared with every reader
			// that already selected this channel, so the per-key status update
			// is applied to a private clone and republished rather than
			// written in place. The per-channel polling lock is kept so a key
			// disable and a concurrent key selection still agree on which keys
			// are usable.
			...
			if beforeStatus != afterStatus {
				// Republishes once more, but this is the call that also drops
				// the channel out of the routing table when its last key died.
				CacheUpdateChannelStatus(channelId, afterStatus)
			}
		} else {
