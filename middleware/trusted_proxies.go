...
func ConfigureTrustedProxies(engine *gin.Engine) error {
	...
	if rawTrustedProxies == "" {
		common.SysError("WARNING: TRUSTED_PROXIES is unset or blank; trusting loopback, RFC 1918, and IPv6 ULA proxy addresses for compatibility. Set TRUSTED_PROXIES=none to trust no proxies, or configure explicit proxy IPs/CIDRs to replace these defaults.")
		common.SysLog("trusted proxies (compatibility defaults): " + strings.Join(defaultTrustedProxyCIDRs, ", "))
		return engine.SetTrustedProxies(defaultTrustedProxyCIDRs)
	}
	if strings.EqualFold(rawTrustedProxies, "none") {
		common.SysLog("trusted proxies: none (X-Forwarded-For and X-Real-IP are ignored)")
	...
	common.SysLog("trusted proxies (from TRUSTED_PROXIES): " + strings.Join(trustedProxies, ", "))
	return nil
}
