...
export function Wallet(props: WalletProps) {
  ...
  const { processing: pancakeProcessing, processWaffoPancakePayment } =
    useWaffoPancakePayment()
  const paymentConfirmationControllerRef = useRef<ReturnType<
    typeof createPaymentConfirmationController
  > | null>(null)
  if (!paymentConfirmationControllerRef.current) {
    paymentConfirmationControllerRef.current =
      createPaymentConfirmationController({
        calculatePaymentAmount,
        invalidatePaymentAmount,
        setConfirmation: setPaymentConfirmation,
        setLoading: setPaymentLoading,
      })
  }
  const paymentConfirmationController = paymentConfirmationControllerRef.current
  ...
  useEffect(() => {
    if (topupInfo && !topupAmountInitializedRef.current) {
      ...
      void paymentConfirmationController.initializeAmount(
        minTopup,
        defaultPaymentType
      )
    }
  }, [topupInfo, paymentConfirmationController])

  // Handle preset selection
  const handleSelectPreset = (preset: PresetAmount) => {
    setTopupAmount(preset.value)
    setSelectedPreset(preset.value)
    void paymentConfirmationController.amountChanged(
      preset.value,
  ...
  const requestPaymentConfirmation = useCallback(
    async (
      ...
      const result = await paymentConfirmationController.select({
        paymentMethod,
        topupAmount,
        waffoMethodIndex,
        loadingKey,
        minTopupAmount: getMinTopupAmount(topupInfo),
        discountRate:
  ...
  const handlePaymentMethodSelect = (method: PaymentMethod) => {
    void requestPaymentConfirmation(method, null, method.type)
  }
  ...
  const handlePaymentConfirm = async () => {
    ...
    if (success) {
      paymentConfirmationController.close()
      await fetchUser()
    }
  }
