...
import { appendStreamDelta } from '../streaming/stream-utils'
...
export function processStreamingContent(
  ...
  const fullContent = appendStreamDelta(
...
export function applyStreamingChunk(
  ...
  if (type === 'reasoning') {
    ...
    return {
      ...
      reasoning: {
        ...reasoning,
        content: appendStreamDelta(reasoning.content, chunk),
      },
  ...
  const contentMessage = processStreamingContent(message, chunk)
