...
export async function createApiKey(
  data: ApiKeyFormData
): Promise<ApiResponse<ApiKeyCredential>> {
...
// Rotate an API key and return the replacement credential once.
export async function rotateApiKey(
  id: number,
  proofToken?: string
): Promise<ApiResponse<ApiKeyCredential>> {
  const res = await api.post(`/api/token/${id}/rotate`, undefined, {
