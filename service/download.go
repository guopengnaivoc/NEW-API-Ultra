...
func DoDownloadRequest(originUrl string, reason ...string) (resp *http.Response, err error) {
	source := types.NewURLFileSource(originUrl)
	identifier := source.GetIdentifier()

	if system_setting.EnableWorker() {
		common.SysLog(fmt.Sprintf("downloading file from worker: %s, reason: %s", identifier, strings.Join(reason, ", ")))
		req := &WorkerRequest{
			URL: originUrl,
			Key: system_setting.WorkerValidKey,
		}
		response, requestErr := DoWorkerRequest(req)
		if requestErr != nil {
	...
	if err := ValidateSSRFProtectedFetchURL(originUrl); err != nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryAccessRejected, source)
	}

	common.SysLog(fmt.Sprintf("downloading from origin: %s, reason: %s", identifier, strings.Join(reason, ", ")))
	response, requestErr := GetSSRFProtectedHTTPClient().Get(originUrl)
	if requestErr != nil {
		return nil, types.NewFileSourceError(types.FileSourceErrorCategoryDownloadFailed, source)
	}
	return response, nil
