...
func (a *Adaptor) DoRequest(c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (any, error) {
	...
	requestCtx, cancel := requestContextForPolling(c)
	defer cancel()
	...
	// 轮询检查消息是否完成
	for {
		err, isComplete := checkIfChatComplete(a, c, info, requestCtx)
		if err != nil {
			return nil, err
		} else {
			if isComplete {
		...
		if err = sleepWithContext(requestCtx, cozePollInterval); err != nil {
			return nil, err
		}
	}
	// 发送获取消息请求
	return getChatDetail(a, c, info, requestCtx)
}
...
func requestContextForPolling(c *gin.Context) (context.Context, context.CancelFunc) {
	...
	return context.WithTimeout(requestCtx, maxDuration)
}
