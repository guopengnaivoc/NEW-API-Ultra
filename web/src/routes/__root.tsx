...
import { buildSessionRecoverySignInHref } from '@/features/auth/lib/auth-redirect'
    ...
    () =>
      subscribeAuthSessionEvents((event) => {
        ...
        if (currentSID && event.sid === currentSID) {
          const target = buildSessionRecoverySignInHref(
...
// Avoid duplicate checks only after this runtime observes setup completion.
...
export const Route = createRootRouteWithContext<{
  ...
  beforeLoad: async ({ location }) => {
    ...
    if (needsSetupCheck) {
      ...
      if (status?.success !== true || status.data?.status !== true) {
