...
const (
	...
	officialRatioPresetHost     = "basellm.github.io"
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	if len(req.Upstreams) > 0 {
		for _, u := range req.Upstreams {
			if isSupportedUpstreamScheme(u.BaseURL) {
				if u.Endpoint == "" {
		...
		for _, ch := range dbChannels {
			if base := ch.GetBaseURL(); isSupportedUpstreamScheme(base) {
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			// An operator's own channel may legitimately answer over plaintext
			// inside a private network, but the presets this project ships
			// point at public pricing documents that end up driving billing,
			// so those get the stricter source rule.
			if isShippedMetadataPresetHost(fullURL) {
				if err := checkUpstreamMetadataSource(fullURL); err != nil {
					logger.LogWarn(c.Request.Context(), "rejected upstream metadata source "+fullURL+": "+err.Error())
			...
			// Verified before any parse, so a pinned document that was
			// tampered with never reaches the ratio comparison.
			if err := checkUpstreamMetadataBody(fullURL, bodyBytes); err != nil {
				logger.LogWarn(c.Request.Context(), "rejected upstream metadata body from "+chItem.Name+": "+err.Error())
				ch <- upstreamResult{Name: uniqueName, Err: err.Error()}
				return
			}
