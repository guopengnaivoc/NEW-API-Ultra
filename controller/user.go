...
var (
	...
	errCreateDefaultToken   = errors.New("create default token")
)
...
func setupLogin(user *model.User, c *gin.Context) {
	...
	twoFAEnabled, err := model.IsTwoFAEnabled(currentUser.Id)
	if err != nil {
		common.SysLog(fmt.Sprintf("Login failed to load 2FA status for user %d: %v", currentUser.Id, err))
		common.ApiErrorI18n(c, i18n.MsgDatabaseError)
		return
	}
	if twoFAEnabled {
		expiresAt := time.Now().Add(5 * time.Minute)
		payload, err := common.Marshal(twoFALoginFlowPayload{AuthVersion: currentUser.AuthVersion})
		if err != nil {
			common.ApiError(c, err)
			return
		}
	...
	setupLoginAtAuthVersion(currentUser, currentUser.AuthVersion, c)
}
...
func setupLoginAtAuthVersion(user *model.User, expectedAuthVersion int64, c *gin.Context) {
	...
	service.WriteRefreshCookie(c, bundle.RefreshToken, bundle.RefreshExpiresAt)
	setAuthNoStore(c)
...
func Register(c *gin.Context) {
	...
	// An address the caller supplies is always honored, whether or not the
	// administrator made verification mandatory. Previously it was dropped
	// without a word when verification was off, so the caller believed the
	// account had a recovery address it never actually got, and registrations
	// deduplicated on username alone could leave two accounts claiming the same
	// mailbox. Storing it still demands a verified code, so an unverified or
	// someone else's address can never be attached.
	registerWithEmail := common.EmailVerificationEnabled || user.Email != ""
	if registerWithEmail {
		if user.Email == "" || user.VerificationCode == "" {
			common.ApiErrorI18n(c, i18n.MsgUserEmailVerificationRequired)
			return
		}
		if err := model.ValidateEmailVerificationFlow(user.VerificationCode, user.Email); err != nil {
	...
	createAccount := func(tx *gorm.DB) error {
		if err := cleanUser.InsertWithTx(tx, inviterId); err != nil {
			return err
		}
		if defaultToken != nil {
			defaultToken.UserId = cleanUser.Id
			if err := tx.Create(defaultToken).Error; err != nil {
				return fmt.Errorf("%w: %v", errCreateDefaultToken, err)
			}
		}
		return nil
	}
	...
	var createErr error
	if registerWithEmail {
		createErr = model.ConsumeEmailVerificationFlowWithAction(
			user.VerificationCode,
			user.Email,
			createAccount,
		)
	} else {
		createErr = model.DB.Transaction(createAccount)
	}
	if createErr != nil {
		switch {
		case errors.Is(createErr, model.ErrAuthFlowInvalid),
			errors.Is(createErr, model.ErrAuthFlowExpired),
			errors.Is(createErr, model.ErrAuthFlowConsumed):
			common.ApiErrorI18n(c, i18n.MsgUserVerificationCodeError)
		case errors.Is(createErr, model.ErrEmailAlreadyTaken):
			common.ApiErrorI18n(c, i18n.MsgUserEmailAlreadyTaken)
		case errors.Is(createErr, errCreateDefaultToken):
			common.ApiErrorI18n(c, i18n.MsgCreateDefaultTokenErr)
		default:
			common.SysLog(fmt.Sprintf("registration transaction failed: %T", createErr))
	...
	if defaultTokenKey != "" {
		response["data"] = gin.H{"key": defaultTokenKey}
	}
...
func RotateAccessToken(c *gin.Context) {
	if !middleware.RequireOneTimeSecurityProof(
		c,
	...
	raw, expiresAt, err := model.RotateDashboardAccessToken(identity.UserID, identity.UserAuthVersion)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func RevokeAccessToken(c *gin.Context) {
	...
	identity, _ := middleware.GetSessionAuthIdentity(c)
	if err := model.RevokeDashboardAccessToken(identity.UserID, identity.UserAuthVersion); err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetSelf(c *gin.Context) {
	...
	// The authenticated role comes from the authoritative user row loaded by
	// dashboard authentication. Use it for capabilities so GetSelf remains
...
func AdminClearUserBinding(c *gin.Context) {
	...
	if !middleware.RequireOneTimeSecurityProof(
		...
		fmt.Sprintf("%d:%s", user.Id, bindingType),
		[]string{
			secureVerificationMethodPassword,
			secureVerificationMethod2FA,
...
func DeleteSelf(c *gin.Context) {
	// Account deletion is irreversible for the account owner, so it demands the
	// same fresh proof of possession as rotating a personal access token: a
	// stolen or borrowed session alone must not be able to destroy the account.
	if !middleware.RequireOneTimeSecurityProof(
		c,
...
func EmailBind(c *gin.Context) {
	...
	if err := model.ValidateEmailVerificationFlow(code, email); err != nil {
		common.ApiErrorI18n(c, i18n.MsgUserVerificationCodeError)
		return
	}
	userID := c.GetInt("id")
	...
	loadedUser, err := model.GetUserById(userID, false)
	if err != nil {
		common.SysLog(fmt.Sprintf("email bind user load failed for user %d (%T)", userID, err))
		common.ApiErrorI18n(c, i18n.MsgDatabaseError)
		return
	}
	identity, ok := middleware.GetSessionAuthIdentity(c)
	...
	if err := model.BindEmailToUserWithVerification(loadedUser, email, code); err != nil {
		switch {
		...
		case errors.Is(err, model.ErrAuthFlowInvalid),
			...
			common.ApiErrorI18n(c, i18n.MsgUserVerificationCodeError)
		default:
			common.SysLog(fmt.Sprintf("verified email bind failed for user %d (%T)", loadedUser.Id, err))
			common.ApiErrorI18n(c, i18n.MsgDatabaseError)
		}
		return
	}
	bundle, err := service.AdvanceCurrentSessionToUserVersion(identity, "email_changed")
	if err != nil {
		logger.LogError(
			c.Request.Context(),
			fmt.Sprintf("email bind session advancement failed for user %d (%T)", loadedUser.Id, err),
		)
		service.ClearRefreshCookie(c)
		recordUserSecurityAudit(c, loadedUser.Id, "user.email_change", map[string]interface{}{
			"email_changed": true,
		})
		setAuthNoStore(c)
		c.JSON(http.StatusOK, gin.H{
...
func UpdateUserSetting(c *gin.Context) {
	...
	if req.QuotaWarningType == dto.NotifyTypeEmail && req.NotificationEmail != "" {
		parsedEmail, err := common.ParseSingleEmailAddress(req.NotificationEmail)
		if err != nil {
			common.ApiErrorI18n(c, i18n.MsgSettingEmailInvalid)
			return
		}
		notificationEmail = parsedEmail
	}
