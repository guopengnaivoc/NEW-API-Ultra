...
func syncToCommon() {
	current := config.Snapshot[PerformanceSetting]("performance_setting")
	common.SetDiskCacheConfig(common.DiskCacheConfig{
		Enabled:     current.DiskCacheEnabled,
		ThresholdMB: current.DiskCacheThresholdMB,
		MaxSizeMB:   current.DiskCacheMaxSizeMB,
		Path:        current.DiskCachePath,
	})

	common.SetPerformanceMonitorConfig(common.PerformanceMonitorConfig{
		Enabled:         current.MonitorEnabled,
		CPUThreshold:    current.MonitorCPUThreshold,
		MemoryThreshold: current.MonitorMemoryThreshold,
		DiskThreshold:   current.MonitorDiskThreshold,
	})
}
