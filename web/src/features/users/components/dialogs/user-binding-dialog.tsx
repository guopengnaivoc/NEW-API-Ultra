      ...
      const started = await startVerification(
        async (proofToken?: string) => {
          ...
          if (target.type === 'builtin') {
            response = await adminClearUserBinding(
              userId,
              target.key,
              proofToken
            )
          } else if (target.providerId) {
            response = await adminUnbindCustomOAuth(
              userId,
              target.providerId,
              proofToken
            )
          ...
          toast.success(t('Unbound {{provider}}', { provider: target.label }))
          await fetchData()
          props.onUnbindSuccess?.()
          return response
        },
        {
          scope: 'admin.binding.clear',
          target: proofTarget,
