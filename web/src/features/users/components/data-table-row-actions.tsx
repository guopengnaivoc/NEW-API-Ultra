...
export function DataTableRowActions({ row }: DataTableRowActionsProps) {
    ...
    try {
      await startVerification(
        async (proofToken?: string) => {
          const result = await resetUserPasskey(user.id, proofToken)
          if (!result.success) {
            throw new Error(result.message || t('Failed to reset Passkey'))
          }
  ...
  const handleResetTwoFA = async () => {
    try {
      await startVerification(
        async (proofToken?: string) => {
          const result = await resetUserTwoFA(user.id, proofToken)
          if (!result.success) {
            throw new Error(result.message || t('Failed to reset 2FA'))
          }
