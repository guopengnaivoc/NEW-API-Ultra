...
// Random per-launch token handed to the spawned backend; readiness is only
// declared when a response echoes it, so a local process squatting on the
// port cannot impersonate our server (NA-ISSUE-0087).
const STARTUP_TOKEN = crypto.randomBytes(32).toString('hex');
...
// Check if a server is available with retry logic. When expectedToken is
// set, a response only counts as ready if it echoes our per-launch startup
// token; an HTTP response from an unrelated process keeps retrying until the
// real child binds (or startup fails when it exits on port conflict).
function checkServerAvailability(port, maxRetries = 30, retryDelay = 1000, expectedToken = null) {
  return new Promise((resolve, reject) => {
    ...
    const tryConnect = () => {
      ...
      const req = http.get({
        ...
        req.destroy();
        if (expectedToken) {
          const gotToken = res.headers['x-electron-startup-token'];
          if (gotToken !== expectedToken) {
            console.error(`Port ${port} answered without our startup token; another process may hold the port. Retrying...`);
            if (currentAttempt >= maxRetries) {
              reject(new Error(`Port ${port} is served by a process that is not the New API backend we started`));
            } else {
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    const env = {
      ...process.env,
      PORT: PORT.toString(),
      ELECTRON_STARTUP_TOKEN: STARTUP_TOKEN,
    };
    ...
    checkServerAvailability(PORT, 30, 1000, STARTUP_TOKEN)
      .then(() => {
        console.log(`✓ Backend server is accessible on port ${PORT} and proved its identity`);
        resolve();
      })
      .catch((err) => {
