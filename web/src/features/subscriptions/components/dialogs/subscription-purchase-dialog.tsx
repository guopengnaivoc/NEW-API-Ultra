...
export function SubscriptionPurchaseDialog(props: Props) {
  ...
  const handlePayStripe = async () => {
    ...
    try {
      ...
      if (res.message === 'success' && res.data?.pay_link) {
        if (!openExternalUrl(res.data.pay_link)) {
  ...
  const handlePayCreem = async () => {
    ...
    try {
      ...
      if (res.message === 'success' && res.data?.checkout_url) {
        if (!openExternalUrl(res.data.checkout_url)) {
          toast.error(t('Invalid payment redirect URL'))
          return
        }
        toast.success(t('Payment page opened'))
  ...
  const handlePayWaffoPancake = async () => {
    ...
    try {
      ...
      if (res.message === 'success' && res.data?.checkout_url) {
        if (!isSafeExternalHttpUrl(res.data.checkout_url)) {
          toast.error(t('Invalid payment redirect URL'))
          return
        }
        toast.success(t('Redirecting to payment page...'))
        redirectToExternalUrl(res.data.checkout_url)
      } else {
        toast.error(
          res.message && res.message !== 'success'
            ? res.message
  ...
  const handlePayEpay = async () => {
    ...
    try {
      ...
      if (res.message === 'success' && res.url) {
        if (!isSafeExternalHttpUrl(res.url)) {
          toast.error(t('Invalid payment redirect URL'))
          return
        }
