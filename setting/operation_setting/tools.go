...
func getToolPriceSetting() *ToolPriceSetting {
	return config.Snapshot[ToolPriceSetting]("tool_price_setting")
}
...
func LoadToolPricesFromJSONString(value string) {
	...
	if err != nil {
		common.SysError("序列化工具价格失败: " + err.Error())
		return
	}
	if _, err := config.GlobalConfig.Update("tool_price_setting", map[string]string{
		"prices": string(data),
	}); err != nil {
		common.SysError("发布工具价格失败: " + err.Error())
		return
	}
	RebuildToolPriceIndex()
}
...
func RebuildToolPriceIndex() {
	current := getToolPriceSetting()
	merged := make(map[string]float64, 9+len(current.Prices))
	seedHardcodedToolPrices(merged)
	for k, v := range current.Prices {
...
func SetToolPriceForTest(name string, price float64) {
	current := getToolPriceSetting()
	prices := make(map[string]float64, len(current.Prices)+1)
	for key, currentPrice := range current.Prices {
		prices[key] = currentPrice
	}
	prices[name] = price
	data, err := common.Marshal(prices)
	if err != nil {
		common.SysError("failed to serialize test tool prices: " + err.Error())
		return
	}
	if _, err := config.GlobalConfig.Update("tool_price_setting", map[string]string{
		"prices": string(data),
	}); err != nil {
		common.SysError("failed to publish test tool prices: " + err.Error())
		return
	}
	RebuildToolPriceIndex()
}
...
func DeleteToolPriceForTest(name string) {
	current := getToolPriceSetting()
	prices := make(map[string]float64, len(current.Prices))
	for key, price := range current.Prices {
		if key != name {
			prices[key] = price
		}
	}
