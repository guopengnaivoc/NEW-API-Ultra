...
func ExplicitUserOverrides(userID int) PermissionsMap {
	...
	for permission, effect := range effects {
		actions, ok := result[permission.Resource]
		if !ok {
			actions = map[string]bool{}
			result[permission.Resource] = actions
		}
		actions[permission.Action] = effect == EffectAllow
	}
	return result
}
