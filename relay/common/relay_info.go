...
type TaskRelayInfo struct {
	...
	PublicTaskID string
	// ProviderResultURI is a transient submit result. It is copied into the
	// encrypted task column before persistence and must never be serialized.
	ProviderResultURI string `json:"-"`
	// SuccessResponse is emitted only after the controller has protected and
	// persisted the accepted task. It must never be serialized.
	SuccessResponse *TaskSuccessResponse `json:"-"`
...
func (response *TaskSuccessResponse) Write(c *gin.Context) {
	if response == nil || c == nil {
		return
	}
