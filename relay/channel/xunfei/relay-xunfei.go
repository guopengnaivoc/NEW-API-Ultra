...
func xunfeiStreamHandler(c *gin.Context, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*dto.Usage, *types.NewAPIError) {
	...
	dataChan, err := xunfeiMakeRequest(c.Request.Context(), textRequest, domain, authUrl, appId)
	if err != nil {
...
func xunfeiHandler(c *gin.Context, textRequest dto.GeneralOpenAIRequest, appId string, apiSecret string, apiKey string) (*dto.Usage, *types.NewAPIError) {
	...
	// Range drains the channel completely; it ends only when the producer
	// closes it, so every queued frame is accumulated before assembling the
	// final response. Usage is taken from every frame — the final status==2
	// frame typically carries the totals with an empty text list, and the old
	// code skipped it entirely.
	for frame := range dataChan {
		xunfeiResponse = frame
		usage.PromptTokens += frame.Payload.Usage.Text.PromptTokens
		usage.CompletionTokens += frame.Payload.Usage.Text.CompletionTokens
		usage.TotalTokens += frame.Payload.Usage.Text.TotalTokens
		if len(frame.Payload.Choices.Text) == 0 {
			continue
		}
		content += frame.Payload.Choices.Text[0].Content
	}
	if len(xunfeiResponse.Payload.Choices.Text) == 0 {
		xunfeiResponse.Payload.Choices.Text = []XunfeiChatResponseTextItem{
			{
...
func xunfeiMakeRequest(ctx context.Context, textRequest dto.GeneralOpenAIRequest, domain, authUrl, appId string) (chan XunfeiChatResponse, error) {
	if ctx == nil {
	...
	// The producer closes dataChan when the websocket stream ends; the close
	// is the completion signal. A receive on a closed buffered channel drains
	// every queued frame first, so the final usage-bearing frame (status == 2)
	// cannot be dropped by a racy select against a separate stop channel.
	dataChan := make(chan XunfeiChatResponse, 8)
