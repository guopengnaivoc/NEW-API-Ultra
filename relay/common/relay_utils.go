...
func normalizeTaskDurationValue(value any) (int, bool) {
	if value == nil {
		return 0, false
	}
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		duration = reflected.Int()
	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		unsigned := reflected.Uint()
		if unsigned > MaxTaskDurationSeconds {
			return 0, false
		}
		duration = int64(unsigned)
	case reflect.Float32, reflect.Float64:
		floating := reflected.Float()
		if math.IsNaN(floating) || math.IsInf(floating, 0) || floating != math.Trunc(floating) ||
			floating < 1 || floating > MaxTaskDurationSeconds {
			return 0, false
		}
		duration = int64(floating)
	case reflect.String:
		parsed, err := strconv.ParseInt(strings.TrimSpace(reflected.String()), 10, 64)
		if err != nil {
			return 0, false
		}
		duration = parsed
	...
	if duration < 1 || duration > MaxTaskDurationSeconds {
		return 0, false
	}
	return int(duration), true
}

// ValidateTaskDuration verifies every supported user-controlled duration path
// before it can become an upstream parameter or billing multiplier. It also
// normalizes valid metadata numbers to int so billing and provider payloads use
// the same value.
func ValidateTaskDuration(req TaskSubmitReq) error {
	if req.durationSet && (!req.durationValid || req.Duration < 1 || req.Duration > MaxTaskDurationSeconds) {
		return fmt.Errorf("seconds must be between 1 and %d", MaxTaskDurationSeconds)
	}
	if !req.durationSet && (req.Duration < 0 || req.Duration > MaxTaskDurationSeconds) {
		return fmt.Errorf("seconds must be between 1 and %d", MaxTaskDurationSeconds)
	}

	if req.Seconds != "" {
		parsed, err := strconv.ParseInt(strings.TrimSpace(req.Seconds), 10, 64)
		if err != nil || parsed < 1 || parsed > MaxTaskDurationSeconds {
			return fmt.Errorf("seconds must be between 1 and %d", MaxTaskDurationSeconds)
		}
	}

	if rawDuration, exists := req.Metadata["durationSeconds"]; exists {
		duration, valid := normalizeTaskDurationValue(rawDuration)
		if !valid {
			return fmt.Errorf("seconds must be between 1 and %d", MaxTaskDurationSeconds)
		}
		req.Metadata["durationSeconds"] = duration
	}
	if rawParameters, exists := req.Metadata["parameters"]; exists {
		parameters, ok := rawParameters.(map[string]interface{})
		if !ok || parameters == nil {
			return fmt.Errorf("parameters must be an object")
		}
		if rawDuration, exists := parameters["duration"]; exists {
			duration, valid := normalizeTaskDurationValue(rawDuration)
			if !valid {
				return fmt.Errorf("seconds must be between 1 and %d", MaxTaskDurationSeconds)
			}
