...
func writeUserCache(user *UserBase, createIfMissing bool) error {
	...
	storedSetting, cacheable, err := userSettingCacheValue(user.Setting)
	if err != nil {
		return err
	}
	if !cacheable {
		return common.RedisDelKey(getUserCacheKey(user.Id))
	}
	createIfMissingArg := "0"
	if createIfMissing {
		createIfMissingArg = "1"
	}
...
return 1`
	result, err := common.RDB.Eval(context.Background(), script,
		...
		user.Username, storedSetting, user.CacheSchema, createIfMissingArg, ttl,
	).Int()
	if err != nil {
		return err
	}
