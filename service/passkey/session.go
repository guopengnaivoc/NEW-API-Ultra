...
func CreateSessionDataFlow(purpose string, userID int, sessionID, scope, target string, data *webauthn.SessionData) (string, int64, error) {
	if data == nil {
...
func PopSessionDataFlow(token, purpose string, userID int, sessionID string) (*webauthn.SessionData, string, string, error) {
	...
	return &payload.SessionData, payload.Scope, payload.Target, nil
}
