...
const remoteImageConfigMaxBytes int64 = 64 << 10
...
func GetImageFromUrl(url string) (mimeType string, data string, err error) {
	...
	if err != nil {
		...
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryDownloadFailed, source)
	}
	defer resp.Body.Close()

	// Check HTTP status code
	if resp.StatusCode != http.StatusOK {
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryUnexpectedStatus, source, resp.StatusCode)
	}

	contentType := resp.Header.Get("Content-Type")
	if contentType != "application/octet-stream" && !strings.HasPrefix(contentType, "image/") {
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryInvalidContent, source)
	}
	maxImageSize := int64(constant.MaxFileDownloadMB * 1024 * 1024)

	// Check Content-Length if available
	if resp.ContentLength > maxImageSize {
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryTooLarge, source, constant.MaxFileDownloadMB)
	}
	...
	if err != nil {
		return "", "", types.NewFileSourceError(types.FileSourceErrorCategoryReadFailed, source)
	}
...
func DecodeUrlImageData(imageUrl string) (image.Config, string, error) {
	...
	var readData []byte
	for _, limit := range []int64{8 << 10, 24 << 10, remoteImageConfigMaxBytes} {
