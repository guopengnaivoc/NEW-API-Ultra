...
// maxAffinityValueKeyLength bounds how much client-controlled data is spliced
// verbatim into cache keys. Longer values are replaced by their SHA-256 so
// arbitrary-length or high-entropy client input cannot blow up the Redis/
// memory keyspace, and raw values never exceed a predictable length.
const maxAffinityValueKeyLength = 64

func normalizeAffinityValueForKey(affinityValue string) string {
	if len(affinityValue) <= maxAffinityValueKeyLength {
		return affinityValue
	}
	digest := sha256.Sum256([]byte(affinityValue))
	return "h256:" + hex.EncodeToString(digest[:])
}
...
func buildChannelAffinityCacheKeySuffix(rule operation_setting.ChannelAffinityRule, modelName string, usingGroup string, affinityValue string) string {
	...
	parts = append(parts, normalizeAffinityValueForKey(affinityValue))
...
func GetPreferredChannelByAffinity(c *gin.Context, modelName string, usingGroup string) (int, bool) {
	...
	for _, rule := range setting.Rules {
		...
		if err != nil {
			common.SysError(fmt.Sprintf("channel affinity cache get failed: rule=%s, key_fp=%s, err=%v", rule.Name, affinityFingerprint(affinityValue), err))
			return 0, false
		}
		if found {
...
func RecordChannelAffinity(c *gin.Context, channelID int) {
	...
	if err := cache.SetWithTTL(cacheKey, channelID, time.Duration(ttlSeconds)*time.Second); err != nil {
		common.SysError(fmt.Sprintf("channel affinity cache set failed: key_fp=%s, err=%v", affinityFingerprint(cacheKey), err))
	}
}
