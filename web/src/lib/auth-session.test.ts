...
describe('authentication session coordination', () => {
  ...
  test('a rejected refresh confirms anonymous state and synchronizes sign-out', async () => {
    ...
    assert.deepEqual(
      await createRefreshRunner(runtime, new AbortController().signal)(),
      {
        kind: 'anonymous',
      }
    )
  ...
  test('the refresh request receives the runner deadline signal', async () => {
    const controller = new AbortController()
    let requestSignal: AbortSignal | undefined
    let transientCount = 0
    const runtime: AuthRefreshRuntime = {
      request: async (_expectedSID, signal: AbortSignal) => {
    ...
    const outcome = await createRefreshRunner(runtime, controller.signal)()
  ...
  test('a refresh response cannot restore credentials after a newer auth epoch', async () => {
    const controller = new AbortController()
    const newerBundle: AuthBundle = {
      ...bundle,
      access_token: 'newer-access-token',
      user: { ...bundle.user, id: 84, username: 'newer-user' },
      session: { ...bundle.session, sid: 'session-b' },
    }
    let currentEpoch = 0
    const refreshEpoch = currentEpoch
    const runtime: AuthRefreshRuntime = {
      request: async () => {
        applyAuthBundle(newerBundle, false)
        currentEpoch += 1
        return { status: 200, data: { success: true, data: bundle } }
      },
      getExpectedSID: () => bundle.session.sid,
      ...
      markTransient: () => undefined,
      wait: async () => undefined,
      isCurrent: () => currentEpoch === refreshEpoch,
    }
  ...
  test('an exact reauthentication marker clears stale authentication', () => {
    useAuthStore.getState().auth.setBundle(bundle)

    applyAuthRotation({ reauthentication_required: true })
  ...
  test('reauthentication-like malformed rotation responses remain fail-closed', () => {
    const malformedRotations = [
      { reauthentication_required: false },
      { reauthentication_required: 'true' },
      { reauthentication_required: true, access_token: 'unexpected' },
    ]

    for (const malformed of malformedRotations) {
      useAuthStore.getState().auth.setBundle(bundle)

      assert.throws(
        () => applyAuthRotation(malformed),
        /Invalid authentication rotation response/
      )
      assert.equal(
        useAuthStore.getState().auth.accessToken,
