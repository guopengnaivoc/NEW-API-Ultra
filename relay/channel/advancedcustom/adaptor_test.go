...
func TestAdaptorResponsesToGeminiAddsThoughtSignatureForFunctionCallHistory(t *testing.T) {
	original, err := config.ConfigToMap(model_setting.GetGeminiSettings())
	require.NoError(t, err)
	t.Cleanup(func() {
		updated, updateErr := config.GlobalConfig.Update("gemini", original)
		require.NoError(t, updateErr)
		require.True(t, updated)
	})
	updated, err := config.GlobalConfig.Update("gemini", map[string]string{
