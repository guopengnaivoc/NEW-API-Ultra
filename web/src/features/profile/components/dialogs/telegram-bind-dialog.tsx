...
export function TelegramBindDialog({
  ...
  const {
    ...
    onSuccess: (result) => {
      const response = result as Awaited<ReturnType<typeof startTelegramBind>>
    ...
    try {
      const started = await startVerification(
        async (proofToken?: string) => {
          const response = await startTelegramBind(proofToken)
  ...
  useEffect(() => {
    ...
    const handleBindResult = async (event: MessageEvent<unknown>) => {
      ...
      const refreshOutcome = await refreshAuthentication()
      if (refreshOutcome.kind !== 'authenticated') {
