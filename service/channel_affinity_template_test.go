...
func TestGetPreferredChannelByAffinity_RequestHeaderKeySource(t *testing.T) {
	...
	t.Cleanup(func() {
		updated, updateErr := config.GlobalConfig.Update("channel_affinity_setting", original)
	...
	require.NoError(t, err)
	updated, err := config.GlobalConfig.Update("channel_affinity_setting", map[string]string{
