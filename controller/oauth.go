...
func HandleOAuth(c *gin.Context) {
	...
	// 4. Consume the callback state before any provider request. OAuth callback
	// states are one-shot credentials, including when token exchange fails.
	pendingFlow, err = model.ConsumeAuthFlow(state, consumeMatch)
	if err != nil {
		c.JSON(http.StatusForbidden, gin.H{"success": false, "message": i18n.T(c, i18n.MsgOAuthStateInvalid)})
		return
	}

	// 5. Handle error from provider
	...
	if pendingFlow.Intent == model.AuthFlowIntentBind {
		handleOAuthBind(c, provider, pendingFlow)
		return
	}
...
func handleOAuthBind(c *gin.Context, provider oauth.Provider, pendingFlow *model.AuthFlow) {
	...
	identity, ok := middleware.GetSessionAuthIdentity(c)
	if !ok || identity.UserID != user.Id || identity.SessionID != pendingFlow.SessionId {
		c.JSON(http.StatusForbidden, gin.H{
			"success": false,
			"message": i18n.T(c, i18n.MsgOAuthStateInvalid),
		})
	...
	if genericProvider, ok := provider.(*oauth.GenericOAuthProvider); ok {
		...
		err = model.UpdateUserOAuthBindingWithAuthVersion(
			user.Id,
			genericProvider.GetProviderId(),
		...
		_, err = model.BindBuiltInExternalIdentityWithAuthVersion(
			user.Id,
	...
	bundle, err := service.AdvanceCurrentSessionToUserVersion(identity, "external_identity_bound")
	if err != nil {
		common.ApiError(c, err)
		return
	}
	recordUserSecurityAudit(c, user.Id, "user.external_identity_bind", map[string]interface{}{
...
func findOrCreateOAuthUser(c *gin.Context, providerName string, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string) (*model.User, error) {
	...
	if genericProvider, ok := provider.(*oauth.GenericOAuthProvider); ok {
		...
		err := model.DB.Transaction(func(tx *gorm.DB) error {
			...
			if err := model.ReplaceExternalIdentityWithTx(
				tx,
				providerName,
				oauthUser.ProviderUserID,
				user.Id,
