...
var (
	...
	protectedProxyClients = proxyHTTPClientCache{
		clients: make(map[string]*http.Client),
		aliases: make(map[string]string),
	}
...
func relayRequestTimeout() time.Duration {
	...
	maxSeconds := int(maxRelayRequestTimeout / time.Second)
	if common.RelayTimeout > maxSeconds {
...
// GetSSRFProtectedHTTPClientWithProxy returns a client for fetching an
// arbitrary destination through a channel proxy. While SSRF protection is
// enabled, forward HTTP proxies are rejected because they resolve the original
// hostname outside this process. SOCKS targets are locally resolved, validated,
// and passed to the proxy as IP literals.
func GetSSRFProtectedHTTPClientWithProxy(rawProxyURL string) (*http.Client, error) {
	...
	parsedURL, legacySuffixStripped, err := common.ParseProxyURLRuntime(
		trimmedProxyURL,
	)
	if err != nil {
		return nil, err
	...
	if fetchSetting != nil && fetchSetting.EnableSSRFProtection &&
		config.parsedURL.Scheme != "socks5" &&
		config.parsedURL.Scheme != "socks5h" {
	...
	policy := defaultHTTPTransportPolicy()
	if client, ok := protectedProxyClients.get(trimmedProxyURL, policy); ok {
		return client, nil
	}
	return protectedProxyClients.getOrCreate(
		trimmedProxyURL,
		config,
		policy,
		func() (*http.Client, error) {
			genericClient, clientErr := GetHttpClientWithProxy(
				config.cacheKey,
			)
			if clientErr != nil {
				return nil, clientErr
			}
			switch config.parsedURL.Scheme {
			case "socks5", "socks5h":
			...
			default:
				return newProtectedFetchHTTPClientWithForwardProxyFallback(
					clientTransport(genericClient),
				), nil
			}
		},
...
func InvalidateProxyClient(rawProxyURL string) {
	...
	for _, client := range protectedProxyClients.removeProxy(config.cacheKey) {
		client.CloseIdleConnections()
	}
}
...
func ResetProxyClientCache() {
	...
	for _, client := range protectedProxyClients.reset() {
		client.CloseIdleConnections()
	}
