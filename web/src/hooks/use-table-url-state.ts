...
type UseTableUrlStateReturn = {
  ...
  normalizeGlobalFilter?: (value: string) => string
  // Column filters
...
export function useTableUrlState(
  ...
  const rawGlobalFilter = (search as SearchRecord)[globalFilterKey]
  const globalFilter =
    globalFilterEnabled && typeof rawGlobalFilter === 'string'
      ? rawGlobalFilter
      : ''

  const normalizeGlobalFilter = useCallback(
    (value: string) => (trimGlobal ? value.trim() : value),
    [trimGlobal]
  )
      ...
      ? (updater) => {
          const next =
            typeof updater === 'function'
              ...
              : updater
          const value = normalizeGlobalFilter(next)
          navigate({
            search: (prev) => ({
              ...(prev as SearchRecord),
              [pageKey]: undefined,
  ...
  return {
    ...
    normalizeGlobalFilter: globalFilterEnabled
