...
func TestGetUserModelsExpandsAutoGroupsInConfiguredOrder(t *testing.T) {
	...
	originalGroupRatioSetting, err := config.ConfigToMap(
		config.Snapshot[ratio_setting.GroupRatioSetting]("group_ratio_setting"),
	)
	require.NoError(t, err)
	t.Cleanup(func() {
		require.NoError(t, setting.UpdateAutoGroupsByJsonString(originalAutoGroups))
		require.NoError(t, setting.UpdateUserUsableGroupsByJSONString(originalUsableGroups))
		updated, updateErr := config.GlobalConfig.Update("group_ratio_setting", originalGroupRatioSetting)
	...
	specialGroups, err := common.Marshal(map[string]map[string]string{
		"default": {
			"+:vip":         "VIP 分组",
			"-:unavailable": "",
		},
	...
	updated, err := config.GlobalConfig.Update("group_ratio_setting", map[string]string{
		"group_special_usable_group": string(specialGroups),
	})
	require.NoError(t, err)
	require.True(t, updated)
