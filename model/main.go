...
func CheckSetup() {
	...
	if setup == nil {
		...
		if initialized, _ := systemInitializationExists(DB); initialized {
...
func migrateDB() error {
	...
	if err := migrateUserAccessTokenColumnType(); err != nil {
		return err
	}
	...
	if err := MigrateCustomOAuthProviderSecrets(); err != nil {
		return err
	}
	if err := MigrateOptionSecrets(); err != nil {
		return err
	}
	if err := MigrateTwoFASecrets(); err != nil {
		return err
	}
	if err := MigrateChannelSecrets(); err != nil {
		return err
	}
	if err := MigrateUserSettingSecrets(); err != nil {
		return err
	}
	if err := MigrateTokenKeys(); err != nil {
		return err
	}
	if err := InitializeUserAuthVersions(); err != nil {
		return err
	}
	if err := InitializeDashboardAccessTokens(); err != nil {
		return err
	}
	if err := InitializeExternalIdentityClaims(); err != nil {
		return err
	}
	if err := MigrateTaskProviderResultURISecrets(); err != nil {
		return err
	}
	if err := MigrateTaskPrivateCredentials(); err != nil {
		return err
	}
	if err := MigrateGeminiTaskResultPrivacy(); err != nil {
		return err
	}
	if err := ValidateGeminiTaskResultPrivacy(); err != nil {
		return err
	}
...
func migrateDBFast() error {
	...
	migrations := []struct {
		...
		{&MidjourneyQuotaReservation{}, "MidjourneyQuotaReservation"},
...
PRIMARY KEY (` + "`id`" + `)
	...
	required := []sqliteColumnDef{
		...
		{Name: "enabled", DDL: "`enabled` numeric"},
	...
	return DB.Exec("UPDATE `" + tableName + "` SET `enabled` = 1 WHERE `enabled` IS NULL").Error
}
...
func migrateUserAccessTokenColumnType() error {
	databaseType := common.MainDatabaseType()
	if databaseType == common.DatabaseTypeSQLite {
		return nil
	}
	if !DB.Migrator().HasTable("users") || !DB.Migrator().HasColumn(&User{}, "access_token") {
		return nil
	}
	...
	case common.DatabaseTypePostgreSQL:
		if err := DB.Raw(`SELECT data_type FROM information_schema.columns
			WHERE table_schema = current_schema() AND table_name = ? AND column_name = ?`,
			"users", "access_token").Scan(&columnType).Error; err != nil {
			return fmt.Errorf("failed to inspect users.access_token type: %w", err)
		}
	...
	if strings.TrimSpace(columnType) == "" {
		return errors.New("failed to inspect users.access_token type: column metadata is empty")
	}

	statements := userAccessTokenMigrationStatements(databaseType)
	if !userAccessTokenColumnNeedsMigration(databaseType, columnType) {
		if databaseType != common.DatabaseTypeMySQL {
			return nil
		}
		var residue int
		result := DB.Raw(
			"SELECT 1 FROM users WHERE access_token LIKE ? LIMIT 1",
			"% ",
		).Scan(&residue)
		if result.Error != nil {
			return fmt.Errorf("failed to inspect users.access_token trailing padding: %w", result.Error)
		}
	...
	for _, statement := range statements {
		if err := DB.Exec(statement).Error; err != nil {
			return fmt.Errorf("failed to migrate users.access_token to varchar(128): %w", err)
		}
	}
	return nil
}

func userAccessTokenColumnNeedsMigration(databaseType common.DatabaseType, columnType string) bool {
	columnType = strings.ToLower(strings.TrimSpace(columnType))
	switch databaseType {
	case common.DatabaseTypePostgreSQL:
		return columnType == "character" || columnType == "bpchar" || strings.HasPrefix(columnType, "char(")
	case common.DatabaseTypeMySQL:
		return strings.HasPrefix(columnType, "char(")
	default:
		return false
	}
}

func userAccessTokenMigrationStatements(databaseType common.DatabaseType) []string {
	switch databaseType {
	case common.DatabaseTypePostgreSQL:
		return []string{
			`ALTER TABLE users ALTER COLUMN access_token TYPE varchar(128) USING NULLIF(RTRIM(access_token), '')`,
		}
	case common.DatabaseTypeMySQL:
		return []string{
