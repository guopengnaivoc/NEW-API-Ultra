...
func enableStreamPingForTest(t *testing.T) {
	t.Helper()
	original, err := config.ConfigToMap(operation_setting.GetGeneralSetting())
	require.NoError(t, err)
	t.Cleanup(func() {
		updated, updateErr := config.GlobalConfig.Update("general_setting", original)
...
func TestCopyCodexSSEHeadersHonorsPolicy(t *testing.T) {
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	resp := &http.Response{
		Header: http.Header{
			"X-Reasoning-Included": {"true"},
			"X-Codex-Turn-State":   {"secret-state"},
	...
	assert.Empty(t, recorder.Header().Values("X-Codex-Turn-State"))
}
