	...
	case relayconstant.RelayModeImagesEdits:
		...
		if mf != nil && mf.File != nil {
			...
			for i, fileHeader := range imageFiles {
				...
				h.Set("Content-Disposition", fmt.Sprintf(`form-data; name="%s"; filename="%s"`, common.EscapeMultipartHeaderValue(fieldName), common.EscapeMultipartHeaderValue(fileHeader.Filename)))
			...
			if maskFiles, exists := mf.File["mask"]; exists && len(maskFiles) > 0 {
				...
				// Create a form file with the appropriate content type
				h := make(textproto.MIMEHeader)
				h.Set("Content-Disposition", fmt.Sprintf(`form-data; name="mask"; filename="%s"`, common.EscapeMultipartHeaderValue(maskFiles[0].Filename)))
				h.Set("Content-Type", mimeType)
