...
func palmStreamHandler(c *gin.Context, resp *http.Response) (*types.NewAPIError, string) {
	...
	// The producer queues at most one payload and then finishes, so the
	// channel close is the completion signal: a receive on a closed buffered
	// channel drains the queued payload first, whereas the old separate stop
	// channel raced it and could drop the entire (single-chunk) response.
