...
func TestRelayRequestTimeoutUsesFiniteFallbackAndCap(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "zero uses fallback", config: 0, expected: defaultRelayRequestTimeout},
		{name: "negative uses fallback", config: -1, expected: defaultRelayRequestTimeout},
		{name: "positive is honored", config: 42, expected: 42 * time.Second},
		{name: "excessive is capped", config: 1 << 30, expected: maxRelayRequestTimeout},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			common.RelayTimeout = testCase.config

			assert.Equal(t, testCase.expected, relayRequestTimeout())
			assert.Equal(t, testCase.expected, newRelayHTTPClient(http.DefaultTransport).Timeout)
			assert.Equal(t, testCase.expected, newProtectedFetchHTTPClient().Timeout)
		})
	}
}
