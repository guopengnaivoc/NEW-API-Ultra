...
> 帰属表示のため上流リファレンスを保持しています。元のソースリポジトリは [`QuantumNous/new-api`](https://github.com/QuantumNous/new-api)、歴史的なイメージは `calciumion/new-api:latest` です。上流のクローン/イメージ手順はこの公開版のデプロイ経路ではありません。

> これは安定化した公開スナップショットであり、未解決の問題がすべて解消されたことを示すものではありません。公開サービスの前に正確なタグを固定し、デプロイ検証を完了してください。
...
事前ビルドイメージまたは単一コンテナの手順は [`DEPLOYMENT.md`](./DEPLOYMENT.md) を参照してください。上記の Compose 手順を推奨します。PostgreSQL、Redis、再起動後も維持するセッション/暗号鍵、データ暗号化キーリングを設定できるためです。これらの設定と永続マウントなしで裸の `docker run` コマンドをコピーしないでください。

</details>

---

🎉 デプロイが完了したら、`http://127.0.0.1:3000` にアクセスして使用を開始してください！
...
| `DATA_ENCRYPTION_KEYS` | 可逆シークレット用のルートキーリング（カンマ区切りの `key-id=base64-key`）。デコード後の各キーは必ず 32 バイトで、すべてのノードで同一にします | - |
| `DATA_ENCRYPTION_ACTIVE_KEY_ID` | 新しいエンベロープとルートキー再ラップに使用するキー識別子 | - |
| `DATA_ENCRYPTION_ENABLE` | 保存時暗号化の適用と移行ゲート。文書化された混在バージョン準備期間以外は無効にしないでください | `true` |
| `SQL_DSN` | データベース接続文字列 | - |
...
1. 宝塔パネル（**9.2.0バージョン**以上）をインストールします。
2. 本スナップショットのデプロイは、このリポジトリの
   [`DEPLOYMENT.md`](./DEPLOYMENT.md) または
   [`宝塔チュートリアル`](./docs/installation/BT.md) に従い、`NEW-API-Ultra` を
   clone して `.env` を生成し、このリポジトリの Docker Compose サービスを起動してください。
3. 宝塔アプリケーションストアの **New-API** ワンクリックパッケージと上流イメージは
   参照用であり、このスナップショットのデプロイ経路ではありません。
...
**[デプロイガイド](./DEPLOYMENT.md)** • **[この公開版の Issues](https://github.com/guopengnaivoc/NEW-API-Ultra/issues)** • **[この公開版の Releases](https://github.com/guopengnaivoc/NEW-API-Ultra/releases)** • **[上流ドキュメント](https://docs.newapi.pro/ja/docs)** • **[上流 Issues](https://github.com/Calcium-Ion/new-api/issues)** • **[上流リリース](https://github.com/Calcium-Ion/new-api/releases)**
